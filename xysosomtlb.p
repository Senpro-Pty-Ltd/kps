/* xysosomtlb.p  - SALES ORDER MAINTENANCE LINE DETAIL SUBROUTINE            */
...
/* $Id: xysosomtlb.p,v 1.2 2018/03/13 17:10:02 gbg Exp $: */
...
/* CYB Upgrade 2012.1EE  LAST MODIFIED: 07-MAY-2013 BY: gbg *c1400*   */
/* CYB                   LAST MODIFIED: 07-FEB-2017 BY: gbg *c1595*   */
/* CYB                   LAST MODIFIED: 06-DEC-2017 BY: nac *d4018a*task869*  */
/*-Revision end---------------------------------------------------------------*/
...
define input parameter p_upd_sod_list_pr like sod_list_pr no-undo.
...
define variable pFieldName          as character no-undo.
define variable pFieldValue         as character no-undo.
define variable pFieldDesc          as character no-undo.
...
/*c1400* start added code */
def var vmessage as char.
/*c1400* end   added code */
...
/*DEFINE SHARED FORMS*/
/*debug* {us/so/solinfrm.i} */
/*debug*/ {us/xy/xysolinfrm.i} 
...
setFrameLabels(frame alt-clc:handle).
 ...
 sod__chr09 colon 55 label "Test Method" skip(1) /* Physical Defect */
 sod__chr06 colon 20 label "Wet Down Code"
 sod__chr10 colon 55 label "Test Results" skip(1)
 sod__chr07 colon 20 label "Manufacture Code" skip(1) /* Kilometres in Service" */
 sod__dec01 colon 20 label "OCV"
 sod__chr08 colon 20 label "Manufacture Source" skip(1) /* SG (all cells) */
...
/* ENSURE NECESSARY GENERALIZED CODES RECORDS EXIST */
if this-is-rma then do:
   assign
      pFieldName = "tr_type"
      pFieldValue = "ISS-RMA"
      pFieldDesc = getTermLabel("PSEUDO_TRANS-TYPE_FOR_RMA_RECEIPT",40).
   {us/px/pxgetph.i gpcodxr}
   if not ({us/px/pxfunct.i &FUNCTION = 'IsValidGeneralizedCode' 
                            &PROGRAM = 'gpcodxr.p'
                            &HANDLE = ph_gpcodxr
                            &PARAM = "input pFieldName,
                                      input pFieldValue"})
   then do:
      {us/px/pxrun.i &PROC='processCreate' &PROGRAM='gpcodxr.p'
                     &HANDLE=ph_gpcodxr
                     &PARAM="(input pFieldName,
                              input pFieldValue,
                              input pFieldDesc)"
...
{us/gp/gpbrparm.i &browse=gplu323.p &parm=c-brparm1 &val="siteEntity"} 
{us/gp/gpbrparm.i &browse=gplu323.p &parm=c-brparm2 &val=''}
/* Add the entity filter to cost center browse */
{us/gp/gpbrparm.i &browse=gplu600.p &parm=c-brparm1 &val="siteEntity"} 
{us/gp/gpbrparm.i &browse=gplu600.p &parm=c-brparm2 &val=''} 
...
do on error undo, retry:
   ...
   then do:
      ...
      do on error undo, retry:
         ...
         /* DISPLAY WARNING MESSAGE RETURN FROM SOBTBRB.P */
         if return-msg <> 0 
         then do:
            run display_msg (input return-msg, input 2).
            assign
               s-rb-init = no
               return-msg = 0.
               if not (batchrun or {us/bbi/gpiswrap.i})
               then 
                  pause.
               undo_all2 = false.
               return.
         ...
         if string ( sod_det.sod_due_date ) <> s-cmdval
         then
            sod_det.sod_promise_date = date(s-cmdval).
...
/*cy1022*/    display 0.0 @ sod_det.sod_std_cost with frame d.
      ...
      if sod_det.sod_fr_list <> "" then do:
         ...
         if not available fr_mstr then do:
            ...
            {us/bbi/pxmsg.i &MSGNUM=670 &ERRORLEVEL=3
...
/*d4081>>*/
          editing:
             case frame-field:
                when "sod__chr05" then do:
		   {us/mf/mfnp05.i
		   code_mstr
		   code_fldval
		   " code_domain = global_domain and code_fldname = 'sod__chr05'"
		   code_value
		   sod_det.sod__chr05}

		   if recno <> ? then do:
		      sod_det.sod__chr05 = code_value.
		      display sod_det.sod__chr05 with frame cyb_warr.
		   end.
		end.
                when "sod__chr09" then do:
		   {us/mf/mfnp05.i
		   code_mstr
		   code_fldval
		   " code_domain = global_domain and code_fldname = 'sod__chr09'"
		   code_value
		   sod_det.sod__chr09}

		   if recno <> ? then do:
		      sod_det.sod__chr09 = code_value.
		      display sod_det.sod__chr09 with frame cyb_warr.
		   end.
		end.
                when "sod__chr10" then do:
		   {us/mf/mfnp05.i
		   code_mstr
		   code_fldval
		   " code_domain = global_domain and code_fldname = 'sod__chr10'"
		   code_value
		   sod_det.sod__chr10}

		   if recno <> ? then do:
		      sod_det.sod__chr10 = code_value.
		      display sod_det.sod__chr10 with frame cyb_warr.
		   end.
		end.
		otherwise do:
...
/*c1400* start added code */
          if  can-find(first code_mstr
		      ...
		      and   code_fldname = "sod__chr05")
          and not can-find(first code_mstr
	              where code_domain = global_domain
		      and   code_fldname = "sod__chr05"
		      and   code_value = sod_det.sod__chr05)
	  then do:
              vmessage = "Invalid Mths of Service".
	      {us/bbi/pxmsg.i &MSGNUM= 16006    &ERRORLEVEL=3}
	      next-prompt frame cyb_warr sod_det.sod__chr05.
	      next wloop.
	  end.
          if  can-find(first code_mstr
	              where code_domain = global_domain
		      and   code_fldname = "sod__chr06")
          and not can-find(first code_mstr
	              where code_domain = global_domain
		      and   code_fldname = "sod__chr06"
		      and   code_value = sod_det.sod__chr06)
	  then do:
              vmessage = "Invalid Wet Down Code".
	      {us/bbi/pxmsg.i &MSGNUM = 16007   &ERRORLEVEL=3}
	      next-prompt frame cyb_warr sod_det.sod__chr06.
	      next wloop.
	  end.
          if  can-find(first code_mstr
	              where code_domain = global_domain
		      and   code_fldname = "sod__chr07")
          and not can-find(first code_mstr
	              where code_domain = global_domain
		      and   code_fldname = "sod__chr07"
		      and   code_value = sod_det.sod__chr07)
	  then do:
              vmessage = "Invalid Manufacture Code".
              vmessage = "Invalid KM in service".
	      {us/bbi/pxmsg.i &MSGNUM = 16009   &ERRORLEVEL=3}
	      next-prompt frame cyb_warr sod_det.sod__chr07.
	      next wloop.
	  end.
          if  can-find(first code_mstr
	              where code_domain = global_domain
		      and   code_fldname = "sod__chr08")
          and not can-find(first code_mstr
	              where code_domain = global_domain
		      and   code_fldname = "sod__chr08"
		      and   code_value = sod_det.sod__chr08)
	  then do:
              vmessage = "Invalid Manufacture Source".
              vmessage = "Invalid SG (all cells)".
	      {us/bbi/pxmsg.i &MSGNUM = 16008   &ERRORLEVEL=3}
	      next-prompt frame cyb_warr sod_det.sod__chr08.
	      next wloop.
	  end.
          if  can-find(first code_mstr
	              where code_domain = global_domain
		      and   code_fldname = "sod__chr09")
          and not can-find(first code_mstr
	              where code_domain = global_domain
		      and   code_fldname = "sod__chr09"
		      and   code_value = sod_det.sod__chr09)
	  then do:
              vmessage = "Invalid Test Method".
              vmessage = "Invalid Physical Defect".
	      {us/bbi/pxmsg.i &MSGNUM = 16010   &ERRORLEVEL=3}
	      next-prompt frame cyb_warr sod_det.sod__chr09.
	      next wloop.
	  end.
          if  can-find(first code_mstr
	              where code_domain = global_domain
		      and   code_fldname = "sod__chr10")
          and not can-find(first code_mstr
	              where code_domain = global_domain
		      and   code_fldname = "sod__chr10"
		      and   code_value = sod_det.sod__chr10)
	  then do:
              vmessage = "Invalid Test Results".
	      {us/bbi/pxmsg.i &MSGNUM = 16011   &ERRORLEVEL=3}
	      next-prompt frame cyb_warr sod_det.sod__chr10.
	      next wloop.
	  end.
/*c1400* end   added code */
          hide frame cyb_warr.
...
----------------------------------------------------------------------------*/
   ...
   {us/gp/gprunp.i "gpglvpl" "p" "set_coa_restriction_ver" "(input true, input siteEntity)"}
   /* ACCT/SUB/CC/PROJ VALIDATION */
   ...
   then do:
      ...
      then do:
         ...
         then do:
            ...
            {us/bbi/gprun.i ""sopkall.p"" "(input no)"}
