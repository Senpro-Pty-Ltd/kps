/* xxfrp1009.p - SALES ORDER INVOICE PRINT FOR ENGLISH PRINT CODE "9"          */
...
/* $Id: xxfrp1009.p,v 1.2 2018/03/13 17:09:57 gbg Exp $: */
...
/* CYB       LAST MODIFIED: 10-AUG-2010     BY: gbg *cy1002e*                  */
/*  YB    LAST MODIFIED: 13-SEP-2011    BY:  gbg *c1241*                     */
/* CYB    LAST MODIFIED: 05-SEP-2012    BY:  gbg *c1339*                     */
/* CYB    LAST MODIFIED: 20-DEC-2012    BY:  gbg *c1339a*                    */
/* CYB Upgrade 2012.1EE  LAST MODIFIED: 14-NOV-2012 BY: gbg *d2101*   */
/* CYB       LAST MODIFIED: 05-FEB-2013     BY: gbg *cy1002h*                 */
/* CYB    LAST MODIFIED: 24-MAY-2013    BY:  gbg *c1411*                      */
...
/*c1339a*/ def stream xlog.
/*c1411*/  def stream xllog.

/*c1339* start added code */
def var l_nontax_amt as dec.
def var tax_tr_type as char.
def var ref as char.
def var tax_lines like tx2d_line init 0.
def var result-status as int.
/*c1339* end   added code */
...
/*cy1002*/  and  (so_shipvia >= shipvia and so_shipvia <= shipvia1)
...
/*c1339*  start added code */
      if not can-find(first tx2d_det
		      ...
		      and   tx2d_ref = so_nbr)
      then do:
         /* There is no tax record 13 (shipped) */
         /* So do a tax recalc now              */
         l_nontax_amt  = 0.
	 tax_tr_type = "13".
...
PROCEDURE p_check_locked:
   ...
   if locked so_mstr
   then do:
/*c1411*  start added code */
       find first so_mstr no-lock
       where recid(so_mstr) = l_so_recno
       no-error.
       run llogit (if available so_mstr then so_mstr.so_nbr else "?").
/*c1411*  end   added code */
      /* Consider legal doc number when 'Separate Invoices for Each Shipment' */
...
/*c1339a* start added code */
{us/xx/xxlog.i}

/*c1411*/
procedure llogit.
defin input parameter iptxt as char.


   output stream xllog to 
             value("/tmp/xxfrp1009.llog")
	     append.
	  
      put stream xllog unformatted
          string(today) "|"
          program-name(1) "|"
          global_userid "|"
   ...
   output stream xllog close.
end. /* procedure llogit */
