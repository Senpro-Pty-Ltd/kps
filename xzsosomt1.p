/* xzsosomt1.p - SALES ORDER MAINTENANCE                                      */
/* sosomt1.p - SALES ORDER MAINTENANCE                                        */
/* Copyright 1986 QAD Inc. All rights reserved.                               */
/* $Id: xzsosomt1.p,v 1.2 2018/03/13 17:10:03 gbg Exp $: */
...
/* Revision: 1.106        BY: Katie Hilbert       DATE: 10/05/09  ECO: *Q3RL* */
/* CYB    LAST MODIFIED: 31-MAY-2013    BY:  gbg *c1417*                     */
/*-Revision end---------------------------------------------------------------*/
/******************************************************************************/
...
define new shared variable shp-rcv-by-pack like mfc_logical initial no.
...
do:
   ...
   do:
      ...
      repeat on stop undo, leave stoptrap:
         ...
         {us/so/sosomt02.i}  /* FORM DEFINITIONS FOR SHARED FRAMES A AND B */
...
/*c1417*         {us/bbi/gprun.i ""sosomta1.p"" */
/*c1417*/         {us/bbi/gprun.i ""xzsosomta1.p""
            "(input this-is-rma,
              output return_int,
              output rma-recno)"}
         ...
         if old_billto <> so_bill and not new_order
         then do:
            /* CALCULATE SALES ORDR BALANCE BY APPLYING EXCHANGE RATE ENTERED */
            /* BY USER ON SO HEADER ONLY WHEN BASE CURR = CUSTOMER CURR       */
            /* AND FIXED RATE = YES AND USER ENTERED EXCHANGE RATE. SINCE     */
            /* FINANCIAL SIDE DO NOT KNOW THE USER ENTERED EXCHANGE RATE,     */
            /* SALES ORDER BALANCE IS CALCULATED HERE BY APPLYING EXCHANGE    */           
            /* RATE FROM SALES ORDER AND CALLING PROCEDURE                    */
            /* updateOpenOrderTotal  BY PASSING CUSTOMER CURRENCY INSTEAD     */
            /* OF SALES ORDER CURRENCY.                                       */
            ...
            then do:
               ...
               /* IF BASE CURRENCY AND CUSTOMER CURRENCY ARE SAME AND */
               /* SALES ORDER IN FOREIGN CURRENCY AND SO_FXIED_RATE = YES */
               if base_curr = cm_mstr.cm_curr and
                  so_fix_rate = yes
               then do:
                  for first exr_rate
            ...
            then do:
               ...
               /* REDUCE BALANCE OF SALES ORDER FOR OLD CUSTOMER */
               run updateOpenOrderTotal in hCreditLibrary
                  ( input  old_billto,
                    input  ordertotal,
                    input  0,
                    input  cm_mstr.cm_curr,
                    output iReturnStatus).

               /* INCREASE BALANCE OF SALES ORDER FOR NEW CUSTOMER */
               run updateOpenOrderTotal in hCreditLibrary
                  ( input  so_bill,
                    input  0,
                    input  OrderTotal,
...
/*c1417*            {us/bbi/gprun.i ""sosomta.p"" */
/*c1417*/            {us/bbi/gprun.i ""xzsosomta.p""
               "(input this-is-rma,
                 input rma-recno,
                 input yes)"}
...
/*c1603* start added code >>>>> */
         {us/bbi/gprun.i ""xzsosomtc.p"" "(input this-is-rma)"}
/*c1603* end added code <<<<< */
/*c1603* start deleted code >>>
         {us/bbi/gprun.i ""sosomtc.p"" "(input this-is-rma)"}
*c1603* end added code <<<< */
         ...
         do transaction:
            ...
            then do:
               ...
               exclusive-lock:
                  ...
                  end. /* if sod_qty_ord = sod_qty_ivcd */
