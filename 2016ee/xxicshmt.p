/* xxicshmt.p - Multi-Transaction Shipper Maintenance                         */
/* icshmt.p - Multi-Transaction Shipper Maintenance                           */
/* Copyright 1986 QAD Inc. All rights reserved.                               */
/* $Id: xxicshmt.p,v 1.2 2018/03/13 17:09:58 gbg Exp $: */
/*                                                                            */
...
/* CYB Patch    LAST MODIFIED: 2014-JUN-18      BY: gbg *c1471*               */
...
/* VARIABLE USED TO HOLD VALUE OF SHIP-TO OF MO IN SSM ONLY */
define variable l_moToSite      as   character    no-undo.
define variable v_continue      as   logical      no-undo initial yes.
...
define variable l_allowedit     as   logical      no-undo.
define variable l_pending       as   logical      no-undo.
...
define variable l_imc_readonly as logical     no-undo.
 
l_imc_readonly = no.
 
if i_tr_type = "ISS-UNP" or i_tr_type = "RCT-UNP" or i_tr_type = "RCT-PO" then do transaction:
...
{us/ic/icdssh.i "reference-only"} /* API shipper dataset and temp tables */
...
if c-application-mode = "API" then do:
   ...
   if valid-handle(ApiMethodHandle) then do:
      ...
      run getDataset in ApiMethodHandle (input "dsCmnShipper",
                                         output dataset dsCmnShipper bind).
      lLegacyAPI = false.
   end.
   else do:
      /* Get handle of API controller */
...
/* THE SUBSCRIBE FOR THE STATEMENT BELOW IS AVAILABLE ONLY VIA MO IN SSM */
publish "fetchMoShipTo" (output l_moToSite).
publish "p_chkValidGblShipper"  
   (output l_allowedit,
    output l_pending,
    output table t_sod_lines).         /* AVAIL FRM PENDING INV MAINT */
...
repeat:
   ...
   if c-application-mode = "API" and not lLegacyAPI then do:
      run getNextRecord in apiMethodHandle (input "ttCmnShipper").
      if return-value = {&RECORD-NOT-FOUND} then leave main_blk.
   end. /* if c-application-mode = "API" */
   ...
   repeat with frame aship on endkey undo main_blk, leave main_blk:
      ...
      if c-application-mode = "API" and lLegacyAPI and retry
         then return error return-value.
 
      if c-application-mode = "API" and not lLegacyAPI and retry
         then return {&APP-ERROR-RESULT}.
 
      if c-application-mode <> "API" then
         global_site = input abs_shipfrom.
 
      if c-application-mode = "API" and lLegacyAPI and not available ttShipper
         then undo main_blk, leave main_blk.
 
      if c-application-mode = "API" and not lLegacyAPI and not available ttCmnShipper
         then undo main_blk, leave main_blk.
      ...
      if c-application-mode = "API" then do:
         ...
         if v_upd_addrs then do:
            if lLegacyAPI
            then run enterAbsShipTo.
            else run enterShipTo.
         end.
         run enterInvMov.
      end.
      ...
      prompt-for
         ...
         v_invmov     when (not l_imc_readonly and l_allowedit)
         editing:
         ...
         if not lLegacyAPI then do:
            assign
               {us/mf/mfaiprvl.i abs_id ttCmnShipper.absId}
               {us/mf/mfaiprvl.i v_invmov ttCmnShipper.invMov}
               {us/mf/mfaiprvl.i abs_shipfrom ttCmnShipper.absShipFrom}
                  when (v_upd_addrs)
               {us/mf/mfaiprvl.i abs_shipto ttCmnShipper.absShipTo}
                  when (v_upd_addrs)
            .
         end.
      end.
      /* VALIDATE THE ENTERED ABS_ID ONLY FOR PENDING INVOICE */
      if l_pending
      then do:
         /* LINES WITH CONFIRMED SHIPPER WERE EDITED */
         if not l_allowedit
         then do:
            /* DISPLAY ERROR IF USER TRIES TO GENERATE NEW ID */
            if input abs_id = ""
            then do:
               /* CONFIRMED SHIPPER EXISTS */
               {us/bbi/pxmsg.i &MSGNUM=14382 &ERRORLEVEL=3}
               if c-application-mode <> "API"
               then
               ...
               /* CHECK IF ENTERED SHIPPER ID EXISTS */
               if not can-find (first b_absmstr
                                  where b_absmstr.abs_domain   = global_domain
                                  and   b_absmstr.abs_shipfrom = input abs_mstr.abs_shipfrom
                                  and   b_absmstr.abs_id       = "s" + input abs_mstr.abs_id)
               then do:
                  ...
                  then
                     next-prompt abs_id with frame aship.
                  undo input_blk, retry input_blk.
               end. /* IF CAN-FIND (FIRST b_absmstr */
 
               /* CHECK IF ENTERED SHIPPER ID IS CONF SHIPPER FOR MOD LINES */
               for each t_sod_lines
                  where t_modified = yes
               no-lock:
                  if not can-do(t_ship_list,input abs_id)
                  then do:
                     /* MODIFIED LINES DO NOT EXIST ON THIS SHIPPER */
                     {us/bbi/pxmsg.i &MSGNUM=14381 &ERRORLEVEL=3}
                     if c-application-mode <> "API"
                     then
                        next-prompt abs_id with frame aship.
                     undo input_blk, retry input_blk.
                  end. /* IF NOT CAN-DO ... */
               end. /* FOR EACH t_sod_lines */
            end. /* IF INPUT abs_id <> "" */
         end. /* IF NOT l_allowedit */
         else do:
            /* EITHER NEW LINE WAS ADDED OR LINE WITH NO CONF SHIPPER WAS MOD */
            if input abs_id <> ""            
            then do:
               /* CONFIRMED SHIPPER SHOULD NOT ALLOWED */
               if can-find (first b_absmstr
                               where b_absmstr.abs_domain   = global_domain
                               and   b_absmstr.abs_shipfrom = input abs_mstr.abs_shipfrom
                               and   b_absmstr.abs_id       = "s" + input abs_mstr.abs_id
                               and   substring(b_absmstr.abs_status, 2, 1) = "y")
               then do:
                  /* SHIPPER ALREADY EXISTS */
                  {us/bbi/pxmsg.i &MSGNUM=8278 &ERRORLEVEL=3}
                  if c-application-mode <> "API"
                  then
                     next-prompt abs_id with frame aship.
                  undo input_blk, retry input_blk.               
...
/*c1471*/ or execname = "xxsois.p"
         or execname = "fsrmash.p" )
/*c1471*/ and input abs_id <> ""
      then do:
 
         if i_tr_type = "ISS-SO" then
         do:
            /* Perform custom validation on shipper */
            {us/bbi/gprun.i ""gpshd1br.p"" "(output v_continue)"}
 
            if not v_continue then do:
               if c-application-mode <> "API" then
                  next-prompt abs_id with frame aship.
               undo input_blk, retry input_blk.
            end.      
         ...
         then do:
            ...
            no-lock:
               for first abs_mstr
                  ...
                  and   abs_shipfrom = si_site
                  and   abs_id       = "s" + input abs_id
               no-lock:
               end. /* FOR FIRST abs_mstr THEN */
               if available abs_mstr and c-application-mode <> "API"
               then do:
                  display
                     "" @ abs_id
                  with frame aship.
      ...
      if c-application-mode <> "API" then do:
         ...
         if v_upd_addrs then do:
            if lLegacyAPI then do:
               ...
               assign
                  {us/mf/mfaiset.i absShipfrom ttCmnShipper.absShipFrom}
                  {us/mf/mfaiset.i absShipto   ttCmnShipper.absShipTo}
                  {us/mf/mfaiset.i absId       ttCmnShipper.absId}
                  {us/mf/mfaiset.i sgInvMov    ttCmnShipper.invMov}.
            end.
         end.
         else do:
            if lLegacyAPI then do:
      ...
      then do:
         ...
         then do:
            ...
            */
            ...
            then do:
               {us/bbi/pxmsg.i &MSGNUM=5802 &ERRORLEVEL=3} /* NOT A SALES ORDER SHIPPER */
               undo, retry.
            end.  /* if abs_inv_mov */
      ...
      then do:
         ...
         then do:
            ...
            &FIELDNAME=""ttCmnShipper.absShipfrom""}
            if c-application-mode <> "API" then
               next-prompt abs_shipfrom.
            undo input_blk, retry input_blk.
         end. /* IF NOT CAN-FIND */
         ...
         then do:
            ...
            &FIELDNAME=""ttCmnShipper.absShipto""}
            if c-application-mode <> "API" then
      ...
      then do:
         ...
         then do:
            ...
            &FIELDNAME=""ttCmnShipper.absId""}
               if c-application-mode <> "API" then
                  next-prompt abs_id.
            undo input_blk, retry input_blk.
         end.  /* IF abs_shipto <> (IF l_moToSite <> "" */
         ...
         then do:
            ...
            then do:
               ...
               {us/bbi/pxmsg.i &MSGNUM=25310 &ERRORLEVEL=3
               &MSGARG1=substring(abs_mstr.abs_id,2)
               &FIELDNAME=""ttCmnShipper.absId""}
               p_error = no.
               if c-application-mode <> "API" then
                  next-prompt abs_id.
      ...
      do:
         ...
         then
            ...
            for first sgad_buff
               ...
               and   sgad_is_src           = no
               and   sgad_addr             = (if l_moToSite <> "" 
                                              then
                                                 i_dest_addr
                                              else 
                                                 absShipto)
            no-lock:
            end. /* FOR FIRST sgad_buff */
      ...
      do transaction:
         ...
         do on error undo, return error return-value:
             if c-application-mode = "API" and not lLegacyAPI then do:
               run setCommonDataBuffer in ApiMethodHandle (input "ttCmnShipperTrailer").
            end.
         ...
         /* Confirm shipper */
         if vlApplyTax or execname = "porvis.p" or execname = "poporc.p" then do:
            if v_unconfirm_flg then
               substring(abs_status, 1, 1) = "y".
            else
      ...
      if i_tr_type = "RCT-PO" then do:
         /* Call the hook and assign tax parameters and price */
         /* from po_mstr/pod_det to abs_mstr.                 */
         {us/bbi/gprun.i ""gppordat1.p"" "(input o_recid)"}
      end.
 
      /* Shipment Certification */
      do transaction:
         for first abs_mstr
            where recid(abs_mstr) = o_recid
         no-lock:
            if abs_mstr.abs_id begins "s"
      ...
      /* PRINT SHIPPER */
      if c-application-mode <> "API" and not {us/gp/gpisapi.i} then
...
PROCEDURE ip_refresh_grp:
   ...
   define buffer b_abs_mstr for abs_mstr.
   define variable l_absShipto as character no-undo.
   /* IN SSM, FOR MO i_dest_addr WILL ALWAYS BE ENGINEER SITE TO WHICH */
   /* INVENTORY IS BEING ISSUED TO */
   l_absShipto = (if l_moToSite <> ""
                  then
                     i_dest_addr
                  else
                     absShipto).
...
PROCEDURE enterShipFrom:
   if c-application-mode <> "API" then do:
      ...
      if lLegacyAPI then do:
         ...
         v_save_shipfrom = ttCmnShipper.absShipFrom.
...
PROCEDURE enterShipTo:
   if c-application-mode <> "API" then do:
      ...
      if lLegacyAPI then do:
         ...
         v_save_shipto = ttCmnShipper.absShipTo.
      end. /* if not lLegacyAPI */
   end.
END PROCEDURE.
...
PROCEDURE enterInvMov:
   if c-application-mode <> "API" then do:
      ...
      if lLegacyAPI then do:
         ...
         v_save_invmov = ttCmnShipper.invMov.
      end. /* if not lLegacyAPI */
   end.
END PROCEDURE.
