...
/* $Id: xxrepkup.p,v 1.2 2018/03/13 17:09:59 gbg Exp $: */
...
/* Revision: 1.6.1.12     BY: Kirti Desai          DATE: 10/31/05 ECO: *P467* */
/* $Revision: 1.2 $    BY: Julie Milligan        DATE: 07/03/08 ECO: *R0W0* */
...
form
   site               colon 22
   site1              label {us/t0/t001.i} colon 49 skip
   part               colon 22
   part1              label {us/t0/t001.i} colon 49 skip
   comp1              colon 22
   comp2              label {us/t0/t001.i} colon 49 skip
   wkctr              colon 22
   wkctr1             label {us/t0/t001.i} colon 49 skip
   issue              colon 22
   issue1             label {us/t0/t001.i} colon 49
   reldate            colon 22
   reldate1           label {us/t0/t001.i} colon 49 skip(1)
   netgr              colon 30
...
repeat:
   ...
   /* NOTE, IN 93, MFGUSER IS LARGER THAN IN PREVIOUS MFG/PRO VERSIONS WHERE  */
   /* IT USE TO BE TMPnnnn (where nnnn is a random number).                   */
   /* WITH THE CHANGE TO MFGUSER, THE TEMPORARY NUMBER WILL BE CREATED        */
   /* TO SIMULATE HOW IT WAS CREATED IN PREVIOUS MFG/PRO VERSIONS, USING      */
   /* GLOBAL_USERID INSTEAD OF 'TMP' AND USING THE LAST 4 CHARACTERS OF THE   */
   /* CURRENT MFGUSER FIELD.                                                  */
   if delete_pklst then
      nbr = trim(global_userid) + substring(mfguser,(length(mfguser) - 3),4).
...
/*c1591* end added code <<< */
   ...
   for first rpc_ctrl
      where rpc_domain = global_domain
   no-lock:
   end. /* FOR FIRST rpc_ctrl */
