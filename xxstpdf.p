/* xxstpdf.p  -  STATEMENTS to PDF  FUNCTION                                  */
...
/*c1255*/ define variable vcCustomerx     as character no-undo.
...
define variable vcFilename      as character format "x(40)" no-undo.
define variable vcFolder        as character format "x(40)" no-undo.
...
/*c1255*/ def var vMessage      as char.
...
/*c1255*/ if vDebtor1 = hi_char then vDebtor1 = "".
    ...
    update vtStartDate     label "Run Date" colon 25 skip
           thrs colon 25   label "Time" format "99" ":" 
	   tmin            no-label format "99" skip(1)
           load_sched      label "Continue" colon 25 skip(1)

/*c1255*/  vDebtor         colon 25  
/*c1255*/  vDebtor1        colon 45
           vcEntity        label "Entity" colon 25
/*c1255*   vcMailcc        label "CC Address" colon 25 */
	   vcPrinter       label "Printer" colon 25
	   vcFolder        label "Folder"  colon 25
	   vcFilename      label "File"   colon 25
...
/*c1255*/    Debtor.DebtorCode >= vDebtor and Debtor.DebtorCode <= vDebtor1 and
	    ...
	    where Address.AddressType_ID = AddressType.AddressType_ID no-lock:
            assign vcMailTo = Address.AddressEmail.
	    if vcMailTo <> "" then next DebtorLoop.
        end.

        assign 
/*c1255*/ vcCustomer = if length(vcCustomer) = 0 then Debtor.DebtorCode
	             else vcCustomer + "," + Debtor.DebtorCode.

/*c1255*/  end.  /* each Debtor */

/*c1255* start added code */
       if vcCustomer = ""
       then do:
           vMessage = "No Customer match selection".
	   {us/bbi/pxmsg.i &MSGTEXT=vMessage &ERRORLEVEL=3}
           undo mainloop, retry mainloop.
       end.
       vcCustomerx = if num-entries(vcCustomer) = 1 then vcCustomer 
                       else "Various".
...
/****************** now build the report request ***************************/

        dataset BReportRequest:empty-dataset().

        create tRptRequest.
        assign tRptRequest.RptRequestName             = "Statement: " + vcCustomerx + " - " +  string(vtStartDate)
               tRptRequest.RptRequestDescription      = "Generated report request for Statement of account for customer " + vcCustomerx
               ...
               tRptRequest.RptRequestSaveFolder       = vcFolder
               tRptRequest.RptRequestSaveFileName     = vcFilename
               tRptRequest.RptRequestMailSubject      = "Monthly Statement of Account <" + vcCustomerx + ">"
