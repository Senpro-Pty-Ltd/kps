...
/* $Id: xfrcsoi1.p,v 1.2 2018/03/13 17:09:36 gbg Exp $: */
...
/*grc338*/ {us/xf/xfdefa.i "new"}  /*29-04-15*/
/*grc338*/ {us/xf/xfso10v.i "new"} /*29-04-15*/ /* procedure vars */
...
define new shared variable prog_name        as character
   initial /* grc338 "rcsois.p" */ "xfrcsois.p" no-undo.
...
define variable isAllPackFullyPickedInShipper as logical no-undo.
define variable serialIDNotFullyPicked as character no-undo.
...
define variable l_y_axis         as integer no-undo.
...
repeat:
   ...
   do on error undo CK-CC-HOLD, leave CK-CC-HOLD:
      ...
      /* SINCE FRAME MOVES POSITION SOMETIMES 
         REASSIGNING IT TO ORIGINAL POSTITION */
      if {us/bbi/gpiswrap.i} 
      then 
         frame a:handle:Y = l_y_axis.
      ...
      /* Check whether all pack linked to this shipper is fully picked */
      {us/px/pxrun.i &PROC = 'isAllPackFullyPickedInShipper'
                     &PROGRAM='pasopixr.p'
                     &HANDLE = ph_pasopixr
                     &PARAM = "(input abs_mstr.oid_abs_mstr,
                                output serialIDNotFullyPicked,
                                output isAllPackFullyPickedInShipper)"
                     &NOAPPERROR = true
                     &CATCHERROR = true}

      if not isAllPackFullyPickedInShipper then do:
         /* Serial ID # has not been fully picked */
         run DisplayMessage (input 12060, input 3, input serialIDNotFullyPicked).
         next-prompt abs_id with frame a.
         undo, retry.
      end.
      ...
      /*grc338
      if execname = "rcsois.p"
       */ if execname = "xfrcsois.p"
      then do:
      ...
      on endkey undo CK-CC-HOLD, leave CK-CC-HOLD:
         ...
         if so_fsm_type = "FSM-RO"
            and not can-find(first sfb_det
                             where sfb_domain  = global_domain
                             and   sfb_nbr     = abs_order
                             and   sfb_so_line = integer(abs_line))
         then do:
            /* NO LINES ON THIS CALL ARE READY TO INVOICE  */
            run DisplayMessage (input 422, 
                                input 3, 
                                input '').
            undo CK-CC-HOLD, leave CK-CC-HOLD.
         end. /* IF NOT CAN-FIND(FIRST sfb_det */
         if confirm_mode
            and so__qadc03 = "yes"
            and not(so_cum_acct)
     ...
     with frame b.
      ...
      do transaction:
         ...
         then do:
            ...
            {us/bbi/gprun.i ""gpshdmbr.p"" "(input  abs_recid,
                                             output l_allowed)"}
...
*/         {us/bbi/gprun.i ""xfsois1e.p""
   ...
   then do:
      ...
      /* PRINT INVOICES */
/*grc338*  {us/bbi/gprun.i ""rcsopspr.p""}   *29-04-15*/
/*grc338*/ {us/bbi/gprun.i ""xfrcpspr.p""}
      ...
      then do: 
         ...
         then do:
            ...
            /* Press 'Next' to print Installed Base Post Report */
            {us/bbi/pxmsg.i &MSGNUM=13500 &ERRORLEVEL=1 &MSGBUFFER=msgText}
         ...
         end. /* IF NOT batchrun and .. */
         {us/bbi/gprun.i ""fsprtisb.p"" "(input table ttisb)"}
      end. /* IF l_prtInstBase and l_prtExtRep */
   end. /* IF brazil_ld = no */
