/* xzsosomtc2.p - SO TRAILER UPDATE LOWER FRAME                           */
/* sosomtc2.p - SO TRAILER UPDATE LOWER FRAME                           */
/* Copyright 1986 QAD Inc. All rights reserved.                               */
/* $Id: xzsosomtc2.p,v 1.1 2019/03/06 00:34:18 mfg Exp $: */
/* $Revision: 1.1 $                                                */
...
/* $Revision: 1.1 $    BY: Arun Kumar           DATE: 02/07/07  ECO: *R0C6* */
/* CYB    LAST MODIFIED: 2017-Oct-06    BY: gbg  *c1603*                      */
/*-Revision end---------------------------------------------------------------*/
...
def var vlcont as logical.
/*c1603* end added code <<<<<< */

{us/mf/mfsotrla.i}
{us/so/sosomt01.i}  /* Define shared form for frame d */
...
do transaction on error undo, retry:
   ...
   if (so_prepaid <> 0 ) then do:
      ...
      {us/bbi/gprun.i ""gpcurval.p"" "(input so_prepaid,
                     input rndmthd,
                     output retval)"}
      if (retval <> 0) then do:
...
/*c1603* start added code >>>>> */
   if substring(so_shipvia, 1, 1) = "V" 
   and substring(so_shipvia, 2, 3) <> substring(so_slspsn[1], 4, 3)
   then do:
       vlcont = no.
       {us/bbi/pxmsg.i &MSGNUM=9005 &ERRORLEVEL=2 &CONFIRM=vlcont}
       if not vlcont then do:
