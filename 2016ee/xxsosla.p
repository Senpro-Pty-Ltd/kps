/* xxsosla.p - STAGE LIST RECORD SELECTION ROUTINE                           */
/* sosla.p - STAGE LIST RECORD SELECTION ROUTINE                             */
/* Copyright 1986 QAD Inc. All rights reserved.                               */
/* $Id: xxsosla.p,v 1.2 2018/03/13 17:10:01 gbg Exp $: */
/*                                                                           */
...
/* Revision: 1.27.1.30 BY: Ed van de Gevel    DATE: 03/01/05 ECO: *R00G*     */
/* Revision: 1.27.1.31 BY: Abhishek Jha       DATE: 08/17/05 ECO: *P3Y4*     */
/* Revision: 1.27.1.32 BY: Miguel Alonso      DATE: 06/30/06 ECO: *R071*     */
/* Revision: 1.27.1.35 BY: Xavier Prat        DATE: 07/13/06 ECO: *R079*     */
/* Revision: 1.27.1.36      BY: Vivek Kamath      DATE: 01/16/08 ECO: *P6HV* */
/* Revision: 1.27.1.37      BY: Jean Miller       DATE: 06/20/08 ECO: *R0VG* */
/* Revision: 1.27.1.38      BY: Xavier Prat       DATE: 06/02/09 ECO: *R1JT* */
/* $Revision: 1.2 $   BY: Xavier Prat       DATE: 05/31/10 ECO: *R20X* */
...
define input parameter avoid_aim        like mfc_logical  no-undo.
...
define variable LVKitCompPickQty as decimal no-undo.
define variable LVKitOpenQty as decimal no-undo.
 
/* SHARED VARIABLES FOR CUSTOMER SEQUENCE SCHEDULES */
{us/so/sosqvars.i}
{us/so/sotmpdef.i}
{us/so/soabsdef.i new}
...
do on error undo mainblock, leave mainblock:
   ...
   then do:
      ...
      if not stage_open then do:
         ...
         if auto_all then do:
            ...
            if err_flag <> 0 and err_flag <> 9 then do:
               ...
               {us/bbi/pxmsg.i &MSGNUM=6137 &ERRORLEVEL=4 &MSGARG1=l_msg1}
               abnormal_exit = true.
            ...
            if err_flag <> 0 and err_flag <> 9 then do:
               ...
               /* DOMAIN # IS NOT AVAILABLE */
               {us/bbi/pxmsg.i &MSGNUM=6137 &ERRORLEVEL=4 &MSGARG1=l_msg2}
               abnormal_exit = true.
      ...
      v_unpicked_qty = 0.
      for each abs_mstr  where abs_mstr.abs_domain = global_domain and
      abs_order = sod_nbr and
         abs_line = string(sod_line) and
         abs_item = sod_part and
         abs_qty <> abs_ship_qty no-lock:
 
         {us/ab/absupack.i "abs_mstr" 3 22 "l_abs_pick_qty" }
      ...
      then do:
         ...
         exclusive-lock:
            ...
            assign
               ...
               qad_wkfl.qad_key1 = qad_temp.qad_key1
            ...
            /* IN CASE OF AIM ACTIVATED WE HAVE WRITTEN ADDITIONAL DATA IN */
            /* THE STAGE LIST RECORD. WE ALSO COPY IT HERE                 */
            if {us/wh/whgpwhon.i} then
               assign
                  qad_wkfl.qad_charfld1[1] = qad_temp.qad_charfld1[1]
                  qad_wkfl.qad_charfld1[5] = qad_temp.qad_charfld1[5]
                  qad_wkfl.qad_charfld1[6] = qad_temp.qad_charfld1[6]
                  qad_wkfl.qad_charfld1[7] = qad_temp.qad_charfld1[7]
                  qad_wkfl.qad_charfld1[8] = qad_temp.qad_charfld1[8]
                  qad_wkfl.qad_charfld1[9] = qad_temp.qad_charfld1[9]
                  qad_wkfl.qad_decfld[5] = qad_temp.qad_decfld[5].
...
/*c1343*            {us/bbi/gprun.i ""soskit01.p"" */
/*c1343*/   {us/bbi/gprun.i ""xxsoskit01.p""
               "(input recid(so_mstr),
                 input recid(sod_det),
                 input kit_all,
                 input part_qty,
            ...
            if ship_avail_qty then do:
               ...
               find qad_wkfl where qad_wkfl.qad_domain = global_domain and
                  qad_key1 = SessionUniqueID + global_db + "stage_list"
            ...
            /* IF WE HAVE SELECTED STAGE OPEN QUANTITY TO NO, THE SYSTEM      */
            /* TRIED TO PICK AVAILABLE COMPONENT INVENTORY. HOWEVER THE       */
            /* SYSTEM DID ALSO CREATE STAGE LIST RECORDS EVEN IF NO INVENTORY */
            /* WAS PICKED. THIS IS NOT CORRECT AND WE NEED TO FIX IT HERE BY  */
            /* DELETING THOSE RECORDS                                         */
            if 
	    /* TEMP_FIX {us/wh/whgpwhon.i} and */
	    stage_open = false then do:
 
               for each qad_wkfl where qad_domain = global_domain
                  and qad_key1 = SessionUniqueID + global_db + "stage_component"
                  and qad_key2 begins sod_nbr + "," + string(sod_line)
               exclusive-lock:
 
                  /* WE NEED TO SEE IF THERE IS PICKED INVENTORY. IF IT IS */
                  /* NOT THE CASE, WE JUST DELETE THE STAGE LIST           */
                  run procValidKitCompPick(qad_key3,
                     qad_key4,
                     qad_charfld[2],
                     qad_charfld[1],
                     qad_charfld[3],
                     qad_charfld[4],
                     qad_charfld[5],
                     output LVKitCompPickQty).
 
                  if LVKitCompPickQty < qad_decfld[1]
                     or qad_decfld[1] <= 0
                  then
                     delete qad_wkfl.
               end.
 
               /* DELETING QAD_WKFL RECORDS IF THERE IS NO COMPONENT INVENTORY */
               /* ALLOCATED FOR KIT ITEMS                                      */
               ...
               /* CALCULATE NEW PARENT QUANTITY. SINCE THE SYSTEM SET THE  */
               /* PARENT QUANTITY TO THE OPEN QUANTITY, THIS MEANS THAT IT */
               /* MIGHT NOT PICK ANYTHING BECAUSE THE SYSTEM WILL BELIEVE  */
               /* THAT THE OPEN QUANTITY IS ZERO                           */
               else do:
 
                  run procKitOpenQty(input recid(sod_det), output LVKitOpenQty).
                  ...
                  exclusive-lock no-error.
                  if available qad_wkfl and
                     qad_decfld[1] <> max(part_qty - LVKitOpenQty,0)
                  then
                     qad_decfld[1] = max(part_qty - LVKitOpenQty,0).
               end.
            end.
      ...
      if not config or open_qty <> 0 then do:
        ...
        then do:
           ...
           /* DE-ACTIVATE THIS PIECE OF CODE WHEN RUN AIM */
           if avoid_aim then do:
      ...
      then do:
         ...
         if available cmf_mstr and v_isqueued then do:
            ...
            {us/bbi/gprun.i ""gpquedoc.p"" "(input-output v_doc_type,
            ...
            if V_trqid <> 0 then do:
               {us/bbi/pxmsg.i &MSGNUM=2834 &ERRORLEVEL=1}
               leave mainblock.
            end.
            v_isqueued = no.
         end.
         ...
         if v_transnbr = -1 then do:
            {us/bbi/pxmsg.i &MSGNUM=2835 &ERRORLEVEL=1}
            leave mainblock.
         end.
      end.
...
PROCEDURE get_shipgrp_attrib:
   ...
   do:
      ...
      if inv_mov <> "" then do:
         {us/bbi/gprun.i ""gpsimver.p"" "(input ship_from,
                                   input inv_mov,
                                   output authorized)"}.
...
/* WAREHOUSING KITS INTERNAL PROCEDURES */
{us/wh/whsoktip.i}
