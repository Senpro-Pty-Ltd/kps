...
/* CYB       LAST MODIFIED: 19-Apr-2013     BY: gbg *c1386*                   */
...
/*c1603*  {us/so/soivpst.i "shared"} */
/*c1603*/ {us/xx/xxivpst.i "shared"}
...
/*cy1002c*/ {us/xf/xfso10u.i}
define variable txresult         as int.
...
/*cy1002c*/   and  (so_site >= site and so_site <= site1)
/*cy1002c*/   and  (so_shipvia >= shipvia and so_shipvia <= shipvia1)
...
/*c1603* start added code >>>> */
          {us/bbi/gprun.i ""xxtx13rm.p"" "(recid(somstr),
                                      output txresult)"}
          if txresult <> 0 then do:
              next subloop.
          end.
/*c1603* end added code <<<<< */
...
/*c1386* start added   code */
/*cy1002c>>*/
         find first Debtor 
         where Debtor.DebtorCode = so_cust 
         no-lock no-error.
         if lvunpriced then do:
      	    if available Debtor and Debtor.CustomCombo6 <> "Unpriced" then 
	    next subloop.
         end.
         if not outboth then do:
            if not outtype and 
	       available Debtor and Debtor.CustomLong1 <> "" then
	       next subloop.
            if outtype and available Debtor and Debtor.CustomLong1 = "" then
	       next subloop.
         end.
/*cy1002c<<*/
/*c1386* end   added  code */
         ...
         do transaction on error undo , leave:
             ...
             */ 
/*d4081-877  if(not can-do("rcsois.p,xfrcsois.p,xxrcsois.p,rcunis.p,fsshcnmt.p,fssacnmt.p",execname)) then do: */
/*d4081-877*/if(not can-do("rcsois.p,xfrcsois.p,rcauis.p,xxauis.p,xxrcsois.p,rcunis.p,fsshcnmt.p,fssacnmt.p",execname)) then do:
...
*c1603*/
      ...
      if last-of(gltw_cc) then do:
       ...
       /*  down 1 with frame gltwtot. *c1603*/
