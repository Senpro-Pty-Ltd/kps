/* xxpomtb.p - PURCHASE ORDER MAINTENANCE -- ORDER HEADER SUBPROGRAM          */
/* popomtb.p - PURCHASE ORDER MAINTENANCE -- ORDER HEADER SUBPROGRAM          */
/* Copyright 1986 QAD Inc. All rights reserved.                               */
/* $Id: xxpomtb.p,v 1.2 2018/03/13 17:09:58 gbg Exp $: */
/*                                                                            */
...
/*cy1012*/ define variable cyb_yn as logical no-undo.
/*cy1012*/ define variable old_due_date like po_due_date no-undo.
/*cy1012*/ define variable openqty              like mrp_qty      no-undo.
...
do on error undo, retry on endkey undo, leave with frame b:
   ...
   do on error undo, retry:
      ...
      if c-application-mode <> "API"
      then do:
/*cy1012*/ assign old_due_date = po_due_date
/*cy1012*/        cyb_yn = yes.
         set
            po_ord_date po_due_date po_buyer po_bill
            so_job po_contract po_contact po_rmks
...
/*cy1012>>*/
            else if frame-field = "po_due_date"
            then do:
	       if (lastkey = keycode("RETURN") or
               ...
               then do:
	          if not new_po and
		  old_due_date <> input po_due_date
		  then do:
	             update cyb_yn label "Change Due Date on all Lines?"
		     with frame cyb_due centered row 10 overlay side-labels.
		     if cyb_yn then do:
		        for each pod_det where pod_domain = global_domain and
		           pod_nbr = po_nbr:
		           assign pod_due_date = input po_due_date.
   ...
   /* Delete */
      ...
      then do:
         if c-application-mode <> "API" then do:
            ...
            {us/px/pxrun.i &PROC  = 'convertNumericToAlpha' &PROGRAM = 'gplngxr.p'
                           &HANDLE = ph_gplngxr
                           &PARAM = "(input 'cns_ctrl',
                                      input 'cost_point',
                                      input po_consign_cost_point,
                                      output cost_point,
                                      output cost_point_label)"}
               ...
               if not cost_point_valid then do:
                  /* INVALID ENTRY */
                  {us/bbi/pxmsg.i &MSGNUM=4509 &ERRORLEVEL=3 &FIELDNAME=""poCostPoint""}
                  undo, retry.
               end.
 
               {us/px/pxrun.i &PROC  = 'convertAlphaToNumeric' &PROGRAM = 'gplngxr.p'
                              &HANDLE = ph_gplngxr
                              &PARAM = "(input 'cns_ctrl',
                                         input 'cost_point',
                                         input cost_point,
