...
/* $Id: xxprapa2.p,v 1.2 2018/03/13 17:11:18 gbg Exp $: */
/*                                                                            */
...
/* REVISION: 9.1       LAST MODIFIED: 01/12/00   BY: *M0HL* Manish Kulkarni   */
...
/* Revision: 1.22         BY: Niranjan Ranka      DATE: 07/12/01  ECO: *P00L* */
...
/* Revision: 1.33         BY: Shilpa Kamath       DATE: 03/30/06  ECO: *R03R* */
/* Revision: 1.34         BY: Robin McCarthy      DATE: 05/31/06  ECO: *R02F* */
/* Revision: 1.35         BY: Shilpa Kamath       DATE: 06/15/06  ECO: *R05W* */
/* Revision: 1.36         BY: Archana Kirtane     DATE: 05/15/07  ECO: *P5WP**/
/* Revision: 1.39         BY: Katie Hilbert       DATE: 07/10/07  ECO: *R0C6* */
/* Revision: 1.40         BY: Vivek Kamath        DATE: 01/08/07  ECO: *P6D3**/
/* Revision: 1.41         BY: Robin McCarthy      DATE: 01/15/08  ECO: *R08C* */
/* Revision: 1.42         BY: Prajakta Patil      DATE: 09/23/09  ECO: *Q3B8* */
/* CYB   LAST MODIFIED: 13-Mar-2018     BY: gbg  *K10017a*                     */
/*-Revision end---------------------------------------------------------------*/
...
define input parameter in_rqd_req_qty  like rqd_det.rqd_req_qty no-undo.
define input parameter in_rqd_rel_date like rqd_det.rqd_rel_date no-undo.
define input parameter in_rqd_due_date like rqd_det.rqd_due_date no-undo.
...
find pt_mstr
   where pt_mstr.pt_domain = global_domain
    and  pt_part = in_rqd_part
no-lock no-error.
 
find ptp_det
   where ptp_det.ptp_domain = global_domain
    and  ptp_part = in_rqd_part
    and  ptp_site = in_rqd_site
no-lock no-error.
...
assign rqd_acct = ocPurchaseGLCode
       temp_sub = ocPurchaseDivisionCode
       temp_cc  = ocPurchaseCostCentreCode.
...
then do:
   {us/px/pxrun.i &PROC  = 'getDefaultInspectionLocation' &PROGRAM = 'ppitxr.p'
            &HANDLE = ph_ppitxr
            &PARAM =  "(input in_rqd_part,
                        input  in_rqd_site,
                        output rqd_loc)"
            &NOAPPERROR = true
            &CATCHERROR = true}
...
if in_repl_cntr = 1 then do:
   assign
      ...
      rqm_buyer      = (if available(vd_mstr) then vd_buyer else "")
      rqm_daybookset = (if available(vd_mstr) 
                        then 
                           vd_daybookset 
                        else 
                           rqm_daybookset).
