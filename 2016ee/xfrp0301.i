...
/* $Id: xfrp0301.i,v 1.2 2018/03/13 17:09:36 gbg Exp $: */
/*                                                                            */
...
/* Revision: 1.14.2.38    BY: Jean Miller         DATE: 11/14/08  ECO: *R15M* */
...
define variable starteff-lbl as character format "x(26)" no-undo.
define variable endeff-lbl as character format "x(24)" no-undo.
...
define variable profileAttributesAt as integer initial 5 no-undo.
...
{us/po/po03b01.i}
/*grc338 {us/po/po03d01.i} /* DEFINE FOR FRAME C */ */
...
then do:
  ...
  /* grc338 run ip_header_print (input po_cmtindx). */
   run ip_header_print (input po_cmtindx, input po_type, input po_nbr). /*grc338*/
   ...
   with frame c width 80:
      ...
      then do:
         ...
         /* run ip_header_print (input po_cmtindx). */ /*grc338*/
         run ip_header_print (input po_cmtindx, input po_type, input po_nbr). /*grc338*/
      ...
      if printAttributeProfile then do:
         run printProfileAttributes(input pod_nbr, input pod_line).
      end.

        grc338 comment end*/

      {us/xf/xfporp03mb.i} /*grc338*/
      ...
      if available sod_det then do:
         ...
         then do:
          ...
          /*grc338
            {us/bbi/gprun.i ""porp3a02.p"" */
/* grc338*/	     {us/bbi/gprun.i ""xfrp3a02.p""
               "(input """", input 0,
      ...
      grc338 comment end*/
      {us/xf/xfcmtprt.i &type=PO &rectype=D &id=pod_cmtindx} /*grc338*/

      if print_wo_comments then do:
         for first wo_mstr where wo_domain = pod_det.pod_domain
                             and wo_lot    = pod_det.pod_wo_lot
         no-lock:
         /*grc338 {us/gp/gpcmtprt.i &type=RP &id= wo_cmtindx &pos=3} */
            {us/xf/xfcmtprt.i &type=RP &rectype=D &id=wo_cmtindx} /*grc338*/
            for first ro_det where ro_domain  = wo_domain
                               and ro_routing = wo_routing
                               and ro_op      = pod_det.pod_op
            no-lock:
        /*grc338 {us/gp/gpcmtprt.i &type=RP &id= ro_cmtindx &pos=3}*/
               {us/xf/xfcmtprt.i &type=RP &rectype=D &id=ro_cmtindx} /*grc338*/
 ...
 *******************************************************************/
   ...
   /*grc338 {us/po/povteprg.i} */
   {us/xf/xfgetabn.i &abn=vatreg} /*grc338*/
 ...
 *******************************************************************/
   ...
   grc338 comment end*/
   {us/xf/xfporp03m0.i} /*grc338*/
END PROCEDURE.

PROCEDURE printProfileAttributes private:
   define input parameter ip_podnbr  like pod_nbr  no-undo.
   define input parameter ip_podline like pod_line no-undo.

   define variable hasAttributes as logical no-undo.

   {us/px/pxrun.i &PROC       = 'getPOLineProfile'
                  &PROGRAM    = 'capfxr.p'
                  &HANDLE     = ph_capfxr
                  &PARAM      = "(input ip_podnbr,
                                  input ip_podline,
   ...
   empty temp-table tt_attribute_report no-error.
   {us/ca/caatprt.i &printItemLevel=printItemLevelAttributes}

   hasAttributes = can-find (first tt_attribute_report).

   if hasAttributes then do:
      run printProfileAttributesHeader.
   end.
   define variable isPrintCertCategory as logical no-undo.
   isPrintCertCategory = no.

   if groupAttributesByCategory then do:
      for each tt_attribute_report no-lock
         group by tt_attribute_report.certCategory
         by tt_attribute_report.sortingSeq:

         isPrintCertCategory = first-of(tt_attribute_report.certCategory)
                               and tt_attribute_report.certCategory > "".

         run printProfileAttributesContent(buffer tt_attribute_report,
                                           input isPrintCertCategory).
      end.
   end. /* if groupAttributesByCategory */
   else do:
      for each tt_attribute_report no-lock by tt_attribute_report.sortingSeq:

         run printProfileAttributesContent(buffer tt_attribute_report,
                                           input isPrintCertCategory).
      end.
   end.
   if hasAttributes and line-counter <> 1 and line-counter <> page-size then do:
      put skip(1).
   end.

END PROCEDURE.

PROCEDURE printProfileAttributesHeader private:

   define variable separator as character initial "-" no-undo.
   separator = fill(separator, 80 - profileAttributesAt).
   ...
   put skip(1).
   put unformatted getTermLabel("ATTRIBUTES_AND_SPECIFICATIONS", 32) at profileAttributesAt skip.
   put unformatted separator at profileAttributesAt skip.

END PROCEDURE.

PROCEDURE printProfileAttributesContent private:
   define parameter buffer tt_attribute_report for tt_attribute_report.
   define input parameter isPrintCertCategory as logical no-undo.

   if groupAttributesByCategory then do:
      if isPrintCertCategory then do:
         if page-size - line-counter < 2 then do:
            page.
         end.

         put unformatted certCategory at profileAttributesAt skip.
      end.

      put attrLabel        format "x(25)" at profileAttributesAt + 2.
      put specDescription  format "x(48)" skip.
   end.
   else do:
      put attrLabel        format "x(27)" at profileAttributesAt.
      put specDescription  format "x(48)" skip.
   end.
