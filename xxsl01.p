/* xxsl01.p - STAGE LIST SUBPROGRAM - PRINT A PICK LIST                       */
/* sosl01.p - STAGE LIST SUBPROGRAM - PRINT A PICK LIST                       */
/* Copyright 1986 QAD Inc. All rights reserved.                               */
/* $Id: xxsl01.p,v 1.2 2018/03/13 17:09:59 gbg Exp $: */
/*                                                                            */
/* Description:                                                               */
...
/* CYB   LAST MODIFIED: 15-FEB-2018    BY: gbg *K10017*                       */
/*-Revision end---------------------------------------------------------------*/
...
def     shared var absrec as recid.
/*k10017*/ def shared var vorderlist as char.
...
break by qad_charfld[12]            /* SITE (SHIP-FROM) */
   ...
   if so_nbr <> old_sod_nbr then do:
      ...
      if available ad_mstr then do:
         ...
         find first ctry_mstr where ctry_ctry_code = ad_mstr.ad_ctry
         no-lock no-error.
...
/*d3494* start added code >>> */
/*k10017*/ vorderlist = vorderlist + 
                       (if vorderlist = "" then string(abs_recno) else
                        "," + string(abs_recno)).
               absrec = abs_recno.
/*d3494* end added code <<<<< */
               if proc_error
...
/*d3494* end added code <<<<< */
   ...
   then do:
      ...
      /* DISCRETE ORDER LINE */
         ...
         for first fac_ctrl fields( fac_domain fac_so_rec)  where
         ...
         then
            ...
            if tot_qad_decfld[1] = tot_qad_decfld[3] then
               qty_open = if l_create_um
                          then
