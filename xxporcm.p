/* xxporcm.p - PURCHASE ORDER RECEIPT W/ SERIAL NUMBER CONTROL                */
/* poporcm.p - PURCHASE ORDER RECEIPT W/ SERIAL NUMBER CONTROL                */
/* Copyright 1986 QAD Inc. All rights reserved.                               */
/* $Id: xxporcm.p,v 1.2 2018/03/13 17:09:58 gbg Exp $: */
/*                                                                            */
...
{proxy/bersprocessor/apiersprocessorrundef.i} 

define variable error-message as character no-undo format 'x(90)' extent 4.
...
define variable l_conv_to_pod_um             as decimal   no-undo.
define variable l_use_pod_um_conv            as logical   no-undo.
define variable l_trans_conv                 as decimal   no-undo.
define variable l_non_cons_qty               as decimal   no-undo.
define variable l_um                         as character no-undo.
define variable l_lotserial                  as character no-undo.
define variable l_trans-ok                   as logical   no-undo.
...
define variable itemAttrExist   as   character                          no-undo.
define variable apiReturnValue  as   character                          no-undo.
...
repeat:
   ...
   if c-application-mode = "API" then do:
      ...
      no-lock no-error.
      if not available ttPurchaseOrderTrans then 
      do:
         apiReturnValue = "".
         leave main-loop.
      end.
   ...
   do transaction:
      ...
      then do:
         ...
         empty temp-table tt_auto_pod_lot no-error.
         for each pod_det             
         ...
         no-lock:
            ...
            then do:
               l_lotserial = "".
               {us/px/pxrun.i &PROC='setAutoLotNumber' &PROGRAM='porcxr1.p'
                  &PARAM="(buffer pod_det,
                                 input porec,
                                 input-output l_lotserial,
                                 output l_trans-ok)"
                  &NOAPPERROR=true
                  &CATCHERROR=true}
               ...
               then do:
                  ...
                  if not can-find (first   tt_auto_pod_lot
                                     where tt_auto_pod_oid = oid_pod_det)
                  then do:
                     create tt_auto_pod_lot.
                     assign
                        tt_auto_pod_oid    = oid_pod_det
                        tt_auto_pod_lotser = l_lotserial.
                  end. /* IF NOT CAN-FIND (tt_auto_pod_lot */
               end. /* IF return-value = {&SUCCESS-RESULT} */
      ...
      empty temp-table tt_rec_det no-error.

      for each prh_hist
         where prh_domain    = global_domain
         and   prh_nbr       = ordernum
      no-lock:
         for first tt_rec_det
            where tt_receiver   = prh_receiver
         no-lock:
         end. /* FOR FIRST tt_rec_det */
         if not available tt_rec_det
         then do:
            create tt_rec_det.
            assign
               tt_receiver   = prh_receiver
               tt_new        = no.
         end. /* IF NOT AVAILABLE tt_rec_det */      
         ...
         do transaction:
            ...
            then do:
               ...
               then do:
                  ...
                  do on error undo, retry:
                    ...
                    /* as soon as you give lgd_nbr in gpldform.i *
                     * a lgd_mstr is created, with lgd_status as "Temp" *
                     * if a lgd_mstr is not already created in 5.20.1 *
                     * so we should not consider that lgd_mstr        */
                    for first lgd_mstr 
                       where lgd_domain = global_domain 
                         and lgd_type = no
                         and lgd_nbr = c_lgd_nbr
                         and lgd_shipfrom = po_vend
                         and lgd_effdate = d_eff_date 
                         and lgd_status <> "Temp" 
                    no-lock: 
                       if lgd_ex_rate <> exch_rate or
                        lgd_ex_rate2 <> exch_rate2 or 
                        lgd_fix_rate <> po_fix_rate
                       then do:
                          assign
                             exch_rate   = lgd_ex_rate
                             exch_rate2  = lgd_ex_rate2
                             po_fix_rate = po_fix_rate.
                       end.
            ...
            if can-find (first lacd_det where 
               ...
               for each lacd_det where 
                        ...
                        lacd_internal_ref = po_nbr and
                        lacd_shipfrom     = po_vend and
                        lacd_internal_ref_type = {&TYPE_PO} 
               exclusive-lock break by
                        lacd_curr:
               ...
               for each tt_lac_exrate where
                        tt_lac_exrate.tt_lac_curr = po_curr 
               exclusive-lock :
                ...
                * THE PO */
               for each tt_lac_exrate 
                  where tt_lac_exrate.tt_lac_curr <> po_curr
               exclusive-lock:
               ...
               for each tt_lac_exrate no-lock:
                  ...
                  for each lacd_det where 
                           ...
                           lacd_shipfrom     = po_vend and
                           lacd_internal_ref_type = {&TYPE_PO} and
                           lacd_curr = tt_lac_exrate.tt_lac_curr 
                  exclusive-lock:
                  ...
                  do on error undo, return error return-value:
                     {us/bbi/gprun.i ""icshup.p""
                        "(input-output shipnbr,
                          ...
                          input 'RCT-PO', no,
                          input 10, input 20,
                          input '')"} /* SITE AS BLANK SINCE THE PROGRAM IS NOT USED */
                                      /* FOR IMC CODE VALIDATION */
                  end.
            ...
            /* EMPTY ATTRIBUTE TEMP TABLES */
            {us/px/pxrun.i &PROC       = 'emptyTTAttrPfValueAndTTAttrValue'
                     &PROGRAM    = 'caatdxr.p'
                     &HANDLE     =  ph_caatdxr
...
/*cy1011*          {us/bbi/gprun.i ""poporca.p"" */
/*cy1011*/         {us/bbi/gprun.i ""xxporca.p""
                  "(input p_int_ref_type,
                    input table tt_auto_pod_lot by-reference,
         ...
         /* Now run ERS to create the supplier invoice */
         if lv_auto_ers = yes and poc_ers_proc = YES and porec = yes
         then do:
         
      		find first lgd_mstr no-lock
      		where lgd_mstr.oid_lgd_mstr = lgdkey
      		and   lgd_mstr.lgd_status = {&LD_CONFIRMED}  no-error.
      
      		if available lgd_mstr 
      		then do:
      
      			assign icParam = 'YES|NO|NO|' + lgd_shipfrom  + '|' + lgd_shipfrom + 
      							 '|||||||' + lgd_nbr + '|' + lgd_nbr + '|' + 
      							 string(lgd_effdate) + '|' + string(lgd_effdate) + '|LEGALDOCUMENT'.
      
      
      			{proxy/bersprocessor/apiersprocessorrunrun.i}
      
      			hide frame warning-frame. pause 0.
      			
      			display with frame a.
      
      		   /* See us/gp/gpselout.i header for explanation of parameters */
      		   ...
      		   {us/bbi/mfphead.i}     /* 132 columns */
      
      		   {us/po/poldaudit.i}
      		   
      		   {us/mf/mfrtrail.i}
      		end. /* if available lgd_mstr */
      	end.
...
if c-application-mode = "API" then do:
   if apiReturnValue = "" then
      return.
   else
      return error apiReturnValue.
end.
...
------------------------------------------------------------------------------*/
   ...
   exclusive-lock:
      ...
      {us/px/pxrun.i &PROC='isItemSerialized'
               ...
               &NOAPPERROR = true
               &CATCHERROR = true}
      if isSerialized then next preppoddet.
      ...
      then do:
         ...
         if pod_qty_chg <> 0 then do:
               ...
               then do: 
                  ...
                  then do:
                     ...
                     then do:
                        ...
                        l_total = (absolute(sr_qty) * (l_trans_conv) )  + l_total.
            ...
            if is-return = no and sr_qty > 0
            then do:
               if can-find (first tt_attr_pfvalue 
                  where tt_attr_pfvalue.tt_oid_attrpf_mstr
                  = sr_wkfl.oid_sr_wkfl ) 
               then do:
                  /* Check if the item lot or item sublot exist. */
                  {us/px/pxrun.i &PROC       = 'isItemAttrExist'
                                 &PROGRAM    = 'caatdxr.p'
                                 &HANDLE     = ph_caatdxr
                                 &PARAM      = "(input sr_wkfl.oid_sr_wkfl,
                                                 input pod_part,
                                                 ...
                                                 input sr_wkfl.sr_ref,
                                                 input table tt_attr_pfvalue by-reference,
                                                 output itemAttrExist)"
                                 &NOAPPERROR = true
                                 &CATCHERROR = true
                  }

                  if itemAttrExist <> {&NO_ITEM_ATTR_MSTR_EXIST} then do:
                     assign
                        pod_qty_chg    = pod_qty_chg - sr_wkfl.sr_qty
                        sr_wkfl.sr_qty = 0.
                  end.
               end.
            end.
...
------------------------------------------------------------------------------*/
   ...
   find first pod_det
      where recid(pod_det) = inpar_recid 
   no-lock no-error.
