...
/* $Id: xxcsrp07.p,v 1.2 2018/03/13 17:09:55 gbg Exp $: */
...
define new shared variable cssetx2 like sct_sim.
...
define new shared variable csdesc2 like cs_desc format "x(18)".
define new shared variable csmethod2 like cs_method.
define new shared variable cstype2  like cs_type.

define            variable  t1pct      as dec format "->>9.9<<".
define            variable  t2pct      as dec format "->>9.9<<".
define            variable  tdpct      as dec format "->>9.9<<".
define            variable  tot1pb     as dec format "->>>>>>9.9<<<<<<".
define            variable  tot2pb     as dec format "->>>>>>9.9<<<<<<".
define            variable  totdpb     as dec format "->>>>>>9.9<<<<<<".
define            variable  tot1mtl    as dec format "->>>>>>9.9<<<<<<".
define            variable  tot2mtl    as dec format "->>>>>>9.9<<<<<<".
define            variable  totdmtl    as dec format "->>>>>>9.9<<<<<<".
define            variable  tot1std    as dec format "->>>>>>9.9<<<<<<".
define            variable  tot2std    as dec format "->>>>>>9.9<<<<<<".
define            variable  totdstd    as dec format "->>>>>>9.9<<<<<<".


define variable vlabour1      as dec format "->>>>>>9.9<<<<<<".
define variable vlabour2      as dec format "->>>>>>9.9<<<<<<".
define variable vlabourd      as dec format "->>>>>>9.9<<<<<<".
define variable voverhead1    as dec format "->>>>>>9.9<<<<<<".
define variable voverhead2    as dec format "->>>>>>9.9<<<<<<".
define variable voverheadd    as dec format "->>>>>>9.9<<<<<<".
define variable vscrap1       as dec format "->>>>>>9.9<<<<<<".
define variable vscrap2       as dec format "->>>>>>9.9<<<<<<".
define variable vscrapd       as dec format "->>>>>>9.9<<<<<<".
...
*moved to include */
	...
	t1pct       label "~% TOTAL"
	t2qty       label "QUANTITY"
	t2cost      label "STD COST"
	t2costext   label "TOTAL"
	t2pct       label "~% TOTAL"
	tdqty       label "QUANTITY"
	tdcost      label "STD COST"
	tdcostext   label "TOTAL"
...
form skip(1)
   ...
   cssetx2         colon 47 no-label
   csdesc1        colon 25
   csdesc2        colon 47 no-label
   csmethod1     colon 25
   csmethod2     colon 47 no-label
   cstype1       colon 25
   cstype2       colon 47 no-label
   skip(1)
with frame a side-labels width 80 attr-space.
...
/*c1429* start added code */
      ...
      else if frame-field = "cssetx2"
      then do:
         /* FIND NEXT/PREVIOUS RECORD */
         if transtype = "SIM"
         then do:
            {us/mf/mfnp01.i cs_mstr cssetx2 cs_set ""SIM""  "
                 cs_mstr.cs_domain = global_domain and cs_type "  cs_set}
         end. /* IF transtype = "SIM" */
         else
            if transtype = "COST"
            then do:
               {us/mf/mfnp01a.i cs_mstr cssetx2 cs_set ""SIM""  "
                  cs_mstr.cs_domain = global_domain and cs_type "  cs_set}
            end. /* IF transtype = "COST" */
            else do:
               {us/mf/mfnp.i cs_mstr cssetx2  " cs_mstr.cs_domain = global_domain and
                 cs_set "  cssetx2 cs_set cs_set}
            end. /* IF transtype <> "COST" */

         if recno <> ?
         then do:
            cssetx2 = cs_set.
            display cssetx2 with frame a.
            find cs_mstr
               where cs_mstr.cs_domain = global_domain
               and   cs_set            = cssetx2
            no-lock no-error.
            if available cs_mstr
            then
               display cs_desc @ csdesc2
	       cs_method @ csmethod2
	       cs_type @ cstype2
	       with frame a.
            recno = ?.
         end. /* IF recno <> ? */
      end. /* IF frame-field = "cssetx2" */
/*c1429* end added code */
      else do:
         readkey.
         apply lastkey.
...
/*c1429*/      {us/mf/mfquoter.i cssetx2      }
/*c1428*      {us/mf/mfquoter.i cssetx1      } */
...
/*c1429* start added code */
         ...
         then do:
            ...
            then
               ...
               next-prompt cssetx2 with frame a.
            undo, retry.
         end. /* IF cssetx2 = "" */
...
/*c1429* start added code */
	...
	assign
     ...
     and   sct_sim = cssetx2:
	assign
        vlabour2 = vlabour2 + sct_lbr_tl.
        voverhead2 = voverhead2 + sct_ovh_tl.
     end.
...
/*debug*
           display
	   tcpart 
	   accum total by tcpart tcqty @ tcqty  
	   tccost  
	   accum total by tcpart tccostext @ tccostext  
	   with frame detx down
	        no-box no-attr-space
	        width 300.
...
*debug*/
	...
	vprevlevel = tclevel.
     ...
     /*  summarise second cost set here */
     vscrap2 = 0.0.
     for each tcost:
        vlabour2 = vlabour2 + tclbr.
	voverhead2 = voverhead2 + tcovh.
     end.
     for each tcost
     where tcleaf or tccostext <> 0.0
     ,
	...
	then do:
	   ...
	   then do:
	      ...
	      assign
	      t2qty = t2qty + (accum total by tcpart tcqty)
	      t2cost = tccost
	      t2costext = t2costext + (accum total by tcpart tccostext).
	      .
   
	   end. /* last-of(tcpart) */
        end.  /*   tcpart begins "zzscarp" ...  else do:  */
     end.  /* each tcost */


     /* calculate totals & percentages */
     vlabourd = vlabour1 - vlabour2.
     voverheadd = voverhead1 - voverhead2.
     vscrapd = vscrap1 - vscrap2.

     for each tresults:
        voverhead1 = voverhead1 + t1ovh.
        voverhead2 = voverhead2 + t2ovh.
        vlabour1 = vlabour1 + t1lbr.
        vlabour2 = vlabour2 + t2lbr.
     ...
     tot2std = vlabour2 + voverhead2 + vscrap2.
     totdstd = vlabourd + voverheadd + vscrapd.
     for each tresults:

	 tdqty = t1qty - t2qty.
	 tdcost = t1cost - t2cost.
	 tdcostext = t1costext - t2costext.

         if tpart begins "pb" 
	 then do:
	    tot1pb = tot1pb + t1costext.
            tot2pb = tot2pb + t2costext.
	    totdpb = totdpb + tdcostext.
	 end.
	 ...
	 then do:
           ...
           tot2mtl = tot2mtl + t2costext.
           totdmtl = totdmtl + tdcostext.
	 end.

         tot1std = tot1std + t1costext.
	 tot2std = tot2std + t2costext.
	 totdstd = totdstd + tdcostext.
     end.
	...
	then do:
     ...
     t2pct = round(vlabour2 / tot2std, 3).
     display 
     " " @ tpart
     ...
     t2pct = round(voverhead2 / tot2std, 3).
     display 
     " " @ tpart
     "Overhead" @ pt_desc1
     " " @ t1qty
     ...
     " " @ t2qty
     " " @ t2cost
     voverhead2 @ t2costext
     t2pct
     " " @ tdqty
     " " @ tdcost
     voverheadd @ tdcostext
     with frame detx1.
     down with frame detx1.

     t1pct = round(vscrap1 / tot1std, 3).
     t2pct = round(vscrap2 / tot2std, 3).
     display 
     " " @ tpart
     vscrapdesc @ pt_desc1
     " " @ t1qty
     ...
     totdstd = tot1std - tot2std.
     t1pct = round(tot1std / tot1std, 3).
     t2pct = round(tot2std / tot2std, 3).
     display
     "----------" @ t1costext
     "----------" @ t2costext
     "----------" @ tdcostext
...
*      if catelm
*      then do:
*   /*c1429*/      {us/bbi/gprun.i ""xxcsrp6b.p"" "(input formula_only)"}
*   /*c1429*      {us/bbi/gprun.i ""bmcsrp1b.p"" "(input formula_only)"} */
*      end.
*      else do:
*   /*c1429*/     {us/bbi/gprun.i ""xxcsrp6a.p"" "(input formula_only)"}
*   /*c1429*      {us/bbi/gprun.i ""bmcsrp1a.p"" "(input formula_only)"} */
*      end.
*c1429* end deleted code */
