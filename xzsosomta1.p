/* xzsosomta1.p - PROCESS SALES ORDER HEADER FRAMES                           */
/* sosomta1.p - PROCESS SALES ORDER HEADER FRAMES                             */
/* Copyright 1986 QAD Inc. All rights reserved.                               */
/* $Id: xzsosomta1.p,v 1.2 2018/03/13 17:10:03 gbg Exp $: */
...
/* Sales Order used by an engineer to obtain inventory needed for service work*/
/******************************************************************************/
...
define variable l_so_exist      like mfc_logical no-undo.
define variable ordertotal        as decimal  no-undo.
define variable creditStatus    as   character initial "HD" no-undo.
define variable ssmCreditStatus as   character initial "HD" no-undo.
...
define variable l_errlevel as   integer initial 0  no-undo. 
   ...
   do transaction on error undo, retry:
      ...
      if not available so_mstr then do:
         ...
         then do:
            ...
            {us/bbi/gprun.i ""gpcfmsg.p"" "(input  8210,           
                                            input  0,              
            ...
            then do:
               if batchrun
               then
                  undo loopx, leave loopx.
               else
                  undo loopx, retry loopx.   
            end. /* IF l_errlevel > 1 */
            else if l_errlevel = 0 
            then do:
               {us/bbi/pxmsg.i &MSGNUM=8210 &ERRORLEVEL=2}
            end. /* ELSE IF l_errlevel = 0 */   
      ...
      if soc_use_corr_inv = true
      then do:
         if (new_order and this-is-rma = false)
            or available cil_mstr
         then do:
            l_so_exist = no.
            run ip_corr_inv
               (output l_so_exist).
         ...
         end. /* IF (new_order AND this-is-rma = FALSE) OR ... */
      end. /* IF soc_use_corr_inv = TRUE */
 
      if corr_inv = no then do:
/*c1453*         {us/bbi/gprun.i ""sosomtcm.p"" */
/*c1453* start deletd code 2016 upgrade >>>>
/*c1453*/   {us/bbi/gprun.i ""xxsomtcm.p""
            "(input  this-is-rma,
              input  recid(rma_mstr),
              input  new_order,
              input  yes,
              output l_edittax)"}
*c1453* end deletd code 2016 upgrade <<<< */
/*c1453* start added code 2016 upgrade */
/*c1453*/   {us/bbi/gprun.i ""xxsomtcm.p""
            "(input  this-is-rma,
              input  recid(rma_mstr),
              input  new_order,
              input  yes,
              output l_edittax)"}
/*c1453* end added code 2016 upgrade */
      ...
      do on error undo, retry with frame b:
         ...
         then ordertotal = getOrderTotal (input so_nbr).
 
         /* CHECK CREDIT LIMIT */
         iValidCustomerCreditLimit =
            validCustomerCreditLimit(bill_cm.cm_addr, ordertotal, 0.0, so_curr, 2, "SO", yes).
 
         if iValidCustomerCreditLimit <> 0 then do:
            creditStatus = getActionStatus("SO").
            ssmCreditStatus = getActionStatus("SSM").
         end.
         ...
         {us/so/sosomtdi.i} /* DISPLAY SO_ORD_DATE, ETC IN FRAME B */
 
         undo_flag = true.
 
/*c1417*         {us/bbi/gprun.i ""sosomtp.p"" */
/*c1417*/   {us/bbi/gprun.i ""xzsosomtp.p""
            "(input this-is-rma,
