/* xxfdopk02.p - DISTRIBUTION ORDER PICKLIST                                 */
/* xxfdopk01.p - DISTRIBUTION ORDER PICKLIST                                 */
/* dsdopk01.p - DISTRIBUTION ORDER PICKLIST                                   */
/* Copyright 1986 QAD Inc. All rights reserved.                               */
/* $Id: xxfdopk02.p,v 1.3 2018/08/02 18:03:36 gbg Exp $: */
/*                                                                            */
...
/* Revision: 1.34     BY: David Morris          DATE: 04/11/07 ECO: *R0C6*    */
/* Revision: 1.35     BY: Jean Miller           DATE: 11/17/07 ECO: *P5NG*    */
/* Revision: 1.36     BY: Xavier Prat           DATE: 06/26/08 ECO: *R0NZ*    */
/* Revision: 1.38    BY: Jean Miller           DATE: 10/07/08  ECO: *R15M*   */
/* $Revision: 1.3 $  BY: Rohan Golatkar        DATE: 12/04/08 ECO: *Q20Q*    */
/* Revision: eb3sp5   BY: Prasannan S           DATE: 25/08/10  ECO: *eb3sp5* */
/* CYB             LAST MODIFIED: 21-JUL-2011   BY: gbg *cy1048*              */
...
{us/xf/xfdopku.i "new"} /*FT User vars*/
{us/xf/xfdopkv.i "new"} /*FT Vars */
/*eb3sp5 END*/
...
form
   ...
   src_site       colon 20
   src_site1      label "To" colon 49 skip
   rec_site       colon 20
   rec_site1      label "To" colon 49 skip
   ord            colon 20
...
if {us/wh/whgpwhon.i} then do:
   ...
   on find of ds_det do:

      find OrderD where OrderD.Domain = global_domain
         and OrderD.DSet = "ds_det"
         and OrderD.Nbr = ds_nbr
         and OrderD.OrdSite = ds_site
         and OrderD.Line = ds_req_nbr
         and OrderD.Part = ds_part no-lock no-error.

      if available OrderD and OrderD.LoadOpt = true then
         return error.

   end. /* on find of ds_det */
end.
...
repeat:
   ...
   then do:
      ...
      find ad_mstr where ad_domain = global_domain and ad_addr = comp_addr
      no-lock no-error.
...
/*eb3sp5*    {us/bbi/mfreset.i} */
/*eb3sp5*/   {us/bbi/mfreset.i "stream ft"}
/*s1087*/    {us/xf/xfoutnulc.i}
...
PROCEDURE p_picklist:
   ...
   define variable l_text as character no-undo.             /*cy1048*/
   ...
   form with frame c down width 80 no-box.
...
/*d2101* start added code >>>>   */
   ...
   if not avail icc_ctrl then do:
      l_text = "Inventory Control file not found".
      {us/bbi/pxmsg.i &MSGTEXT=l_text &ERRORLEVEL=3}
      return.
   end.
/*d2101* end   added code <<<<<  */
...
/*c1342*  changed xxfdopk1.i to xxfdopk01.i */
      /*>>cy1048*/
      if icc_ascend then do:
         case icc_pk_ord:
            when 1 then do:
               {us/xx/xxfdopk01.i &sort1 = "lad_loc"}
            end.
            when 2 then do:
               {us/xx/xxfdopk01.i &sort1 = "lad_lot"}
            end.
            when 3 then do:
               {us/xx/xxfdopk01.i &sort1 = "ld_date"}
            end.
            when 4 then do:
               {us/xx/xxfdopk01.i &sort1 = "ld_expire"}
            end.
         end case.
      end.
      else do:
         case icc_pk_ord:
            when 1 then do:
               {us/xx/xxfdopk01.i &sort1 = "lad_loc" &sort2 = "descending"}
            end.
            when 2 then do:
               {us/xx/xxfdopk01.i &sort1 = "lad_lot" &sort2 = "descending"}
            end.
            when 3 then do:
               {us/xx/xxfdopk01.i &sort1 = "ld_date" &sort2 = "descending"}
            end.
            when 4 then do:
               {us/xx/xxfdopk01.i &sort1 = "ld_expire" &sort2 = "descending"}
            end.
         end case.
      end.  /* descending */
      /*<<cy1048*/   

/*d3494* start added code >>> */
      for each ds_det no-lock
	   ...
	   if update_yn 
	   and last-of(ds_shipdate)
	   and last-of( ds_part)
	   and last(ds_part) then do:
               {us/bbi/gprun.i ""xx349403.p"" "(recid(dss_mstr))"}
	   end.
      end.
/*d3494* and added code <<<< */



/*cy1048* start deleted code  >>>>>>>>>>>>>>>>
      for each ds_det no-lock
          where ds_domain = global_domain
            and ds_nbr = dss_nbr
            and ds_shipsite = dss_shipsite
...
/*d3494* start added code >>> * 
	    ...
	    then do:
               assign
                  dsdet.ds_qty_pick = dsdet.ds_qty_pick + dsdet.ds_qty_all
                  dsdet.ds_qty_all  = 0.
               {us/bbi/gprun.i ""xx349403.p"" "(recid(dsdet))"}
               release dsdet.
