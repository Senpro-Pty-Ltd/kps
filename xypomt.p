/* xypomt.p - PURCHASE ORDER MAINTENANCE - Regular and Blanket POs              */
/* pomt.p - PURCHASE ORDER MAINTENANCE - Regular and Blanket POs              */
/* Copyright 1986 QAD Inc. All rights reserved.                               */
/* $Id: xypomt.p,v 1.2 2018/03/13 17:10:02 gbg Exp $: */
/*                                                                            */
...
/* CYB    LAST MODIFIED: 15-DEC-2011    BY:  gbg *c1262*  MultiDomain        */
...
/*c1262*/ def var prevdomain as char.
/*c1262*/ def var vMessage as char.
      ...
      do on error undo, retry:
         ...
         then do:
            ...
            then do:
               ...
               then do:
                  undo vendblk, retry.
               end. /* IF blockedSupplier(INPUT INPUT po_vend, */
            end. /* END IF pBlanket = TRUE */
            else do:
               /* Normal PO */
               /* Check to see if Supplier has any blocked transactions */
               if blockedSupplier(input input po_vend,
...
/*c1262* start added code */
            if not can-find(first si_mstr
	                    where si_site = input po_ship)
            then do:
                 vMessage = "Ship-To does not exist".
		 {us/bbi/pxmsg.i &MSGTEXT=vMessage &ERRORLEVEL=3}
		 pause.
		 undo, retry.
	    end.
/*c1262* end added code <<<< */
...
*d2101* end   deleted code  <<<<<<<<<<<<<<< */
 ...
 /*
 /*debug*/ message return-value {&SUCCESS-RESULT}. pause.
         if return-value <> {&SUCCESS-RESULT} then do:
            if c-application-mode <> "API" then
               undo, retry.
            else
...
/*c1262*/   prevdomain = global_domain.
/*c1262*/   for first si_mstr no-lock
/*c1262*/   where si_site = input po_ship:
/*c1262*/     global_domain = si_domain.
/*c1262*/   end.
            {us/mf/mfaddisp.i po_ship ship_to}
/*c1262*/ global_domain = prevdomain.
         end.
         else do:
...
/*c1416*  {us/bbi/gprun.i ""popomtb.p"" "(input using_supplier_consignment)"} */
/*c1416*/ {us/bbi/gprun.i ""xypopomtb.p"" "(input using_supplier_consignment)"}
   ...
   /* LINE ITEMS */
/*q9006*    {us/bbi/gprun.i ""popomta.p""} */
/*q9006*/   {us/bbi/gprun.i ""xypopomta.p""}
