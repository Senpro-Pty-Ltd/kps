/* xxrqpobldd.p - Requisition To PO -- Line Items subroutine                  */
/* rqpobldd.p - Requisition To PO -- Line Items subroutine                    */
/* Copyright 1986 QAD Inc. All rights reserved.                               */
/* $Id: xxrqpobldd.p,v 1.2 2018/03/13 17:09:59 gbg Exp $: */
/*!
 *----------------------------------------------------------------------------
...
/* Revision: 1.13.1.41     BY: Reena Ambavi        DATE: 06/28/05 ECO: *P3QY* */
/* Revision: 1.13.1.42     BY: Somesh Jeswani      DATE: 08/03/05 ECO: *P3WK* */
/* Revision: 1.13.1.45     BY: Shilpa Kamath       DATE: 02/22/06 ECO: *R036* */
/* Revision: 1.13.1.46     BY: Changlin Zeng       DATE: 05/16/06 ECO: *R045* */
/* Revision: 1.13.1.48     BY: Robin McCarthy      DATE: 05/31/06 ECO: *R02F* */
/* Revision: 1.13.1.50     BY: Suyash Keny         DATE: 01/18/07 ECO: *P5JL* */
/* Revision: 1.13.1.51     BY: Sumit Karunakaran   DATE: 02/19/07 ECO: *P5MH* */
/* Revision: 1.13.1.52     BY: Ashim Miahra        DATE: 03/02/07 ECO: *P5Q1* */
/* Revision: 1.13.1.54     BY: Arun Nair           DATE: 06/12/07 ECO: *R08C* */
/* Revision: 1.13.1.55     BY: Vivek Kamath        DATE: 01/29/08 ECO: *Q1GG* */
/* Revision: 1.13.1.56     BY: Rajat Kulshreshtha  DATE: 04/15/09 ECO: *Q2QX* */
/* Revision: 1.13.1.62     BY: Avishek Chakraborty DATE: 07/10/09 ECO: *Q2P1* */
/* Revision: 1.13.1.63     BY: Prajakta Patil      DATE: 09/25/09 ECO: *Q3B8* */
/* Revision: 1.13.1.64     BY: Ruchita Shinde      DATE: 10/10/09 ECO: *Q34R* */
/* Revision: 1.13.1.65     BY: Chandrakant Ingale  DATE: 02/04/10 ECO: *Q3PV* */
...
define variable l_validAcct            as logical        no-undo.
define variable l_messageList          as character      no-undo.
define variable l_severityList         as character      no-undo.
define variable ext_cost_fmt1          as character      no-undo.
define variable req-rndmthd            like rnd_rnd_mthd no-undo.
...
/* COPY REQUISITION LINES TO TEMP-TABLE */
/*c1199*  {us/bbi/gprun.i ""rqpobldg.p""} */
/*c1199*/ {us/bbi/gprun.i ""xxrqpobldg.p""}
...
with frame rpt width 132 down:
   ...
   /* SET THE CURRENCY DEPENDENT FORMAT */
   if wkrqd_curr = base_curr then req-rndmthd = gl_rnd_mthd.
   else do:
   ...
   {us/bbi/gprun.i ""gpcurfmt.p""
      "(input-output ext_cost_fmt1,
        input req-rndmthd)"}

   assign
      ext_cost:format in frame po_line_rpt = ext_cost_fmt1
      var_sub    = wkrqd_sub
      var_cc     = wkrqd_cc.
   ...
   if     wkrqd_type <> " "
      and wkrqd_type <> "S"
   then do:
      /* VALIDATE ACCT/SUB/CC CODES AND COMBINATIONS */
      ...
      then do:
         ...
         {us/gp/gprunp.i "gpglvpl" "p" "get_msgs"
            "(output l_messageList,
              output l_severityList)"}

         do i = 1 to num-entries(l_messageList):
            display
               wkrqd_acct
            with frame account.
            run put_message (input integer(entry(i,l_messageList)),
                             input integer(entry(i,l_severityList))).
            run put_message (input 1304, input 4).
            undo rqpoloop, retry.
         end. /* DO i = 1 TO NUM-ENTRIES(l_messageList): */
      end. /* IF NOT l_validAcct */
   end. /* IF wkrqdtype <> "" ... */
   ...
   if not available pod_det then do:
      ...
      assign
         ...
         pod_wo_lot     = wkrqd_wo_lot
         pod_op         = wkrqd_op
         pod__qad16     = wkrqd_sub_type
         pod_wip_lotser = wkrqd_wip_lotser
         pod_cst_up     = yes.
      ...
      if available pt_mstr then
         assign
            ...
            pod_taxc    = if pt_taxc <> "" then pt_taxc else po_taxc.
      else
         /** FOR MEMO ITEMS **/
         assign
      ...
      assign
         ...
         /* REFERENCE: POPOMTI.P */
         pod_rev            = wkrqd_rev
         pod_item_rev_date  = wkrqd_item_rev_date
      ...
      then do:
         ...
         if pod_consignment 
            and po_consign_cost_point = ""
         then do:
            {us/px/pxrun.i &PROC = 'initializeCostPoint'
                     &PARAM="(input po_vend,
                              input-output po_consign_cost_point)"}
         end. /* IF pod_consignment AND .. */                     
      end. /* IF using_consignment AND CAN-FIND(pt_mstr */
      else
         assign
            pod_consignment    = no
            pod_max_aging_days = 0.
            ...
            if rqm_curr <> base_curr then do:
               if rqm_fix_rate then
                  ...
                  if exch-error-number <> 0 then do:
                     /* NO EXCHANGE RATE FOR REQUISITION TO BASE CURRENCY */
      ...
      l_netprice = if wkrqd_um <> var_um  then
                      getDecFromChar(wkrqd_qadc01) * var_um_conv
                   else
                      getDecFromChar(wkrqd_qadc01).
 
      assign
         l_netprice_int  = l_netprice
         l_netprice_frac = (l_netprice - l_netprice_int ) * 100000.
      ...
      then
         ...
         ext_cost = if pod_pur_cost <> 0
                    then
                       pod_qty_ord * (l_netprice_int + l_netprice_frac / 100000)
                    else
                       0.
      else
      ...
      if use-log-acctg
         ...
         and pod_type = ""  /* LOGISTIC ACCT WORKS ONLY WITH INVENTORY ITEM */
      then do:
         ...
         if po_curr = base_curr then do:
            ...
            if unitcost = if (wkrqd_curr = po_curr) then var_pur_cost
                          else var_vend_pur_cost
            then do:
               /* UPDATE LOGISTICS ACCOUNTING TERMS OF TRADE FIELD */
               {us/gp/gprunmo.i &module = "LA" &program = "lapopr.p"
                          &param="""(input  pod_um,
      ...
      then do:
         ...
         {us/bbi/pxmsg.i &MSGNUM=982 &ERRORLEVEL=4
                  &MSGARG1=pod_part
                  &MSGARG2=pod_um}
      end.
      ...
      l_price_cost = 0.
      if var_req_cost <> dummy_cost then
         assign
            var_pur_cost = pod_pur_cost
      ...
      var_req_cost = if pod_disc_pct <> 0 then
                        pod_pur_cost
      ...
      if var_req_cost <> 0 then do:
         ...
         if l_price_cost    <> 0
            and round(var_req_cost,5) > round(l_price_cost,5)
         then do:
            display
               var_req_cost
               l_price_cost @ dummy_cost
      ...
      if include_lcmmts and wkrqd_cmtindx <> 0 then do:
         ...
         for each wkcmt_det
            where wkcmt_det.wkcmt_indx = wkrqd_cmtindx
         no-lock:
            ...
            if i = -1 then do:
               ...
               {us/mf/mfrnseq.i cmt_det cmt_det.cmt_indx cmt_sq01}
...
PROCEDURE put_message:
  ...
  {us/bbi/pxmsg.i &MSGNUM=m_message &ERRORLEVEL=m_err_level
