/* xxcsrp7a.p - BILL OF MATERIAL COST REPORT BY CATEGORY                */
/* bmcsrp1a.p - BILL OF MATERIAL COST REPORT BY CATEGORY                */
/* Copyright 1986 QAD Inc. All rights reserved.                               */
/* $Id: xxcsrp7a.p,v 1.2 2018/03/13 17:09:56 gbg Exp $: */
...
/* $Revision: 1.2 $ BY: Gaurav Kerkar      DATE: 12/14/06 ECO: *P5JD* */
...
define variable mtl_ext_tl like mtl_tl no-undo.
define variable lbr_ext_tl like sub_tl no-undo.
define variable bdn_ext_tl like bdn_tl no-undo.
define variable ovh_ext_tl like ovh_tl no-undo.
define variable sub_ext_tl like sub_tl no-undo.
define variable total_ext_tl like total_tl no-undo.
...
/*c1429* start added code */
def var l_qty_per  as dec extent 20.
def var l_scrp_pct as dec extent 20.
def var l_rate     as dec format "->>>>>>>9.99<<<<<<"      label "        Rate".
def var l_scrp     as dec format "->>>>>>>9.99<<<<<<"      label "       Scrap".
def var qty_tl     as dec format "->>>>>>>9.99<<<<<<"      label "      Qty TL".
def var qty_scrp_tl     as dec format "->>>>>>>9.99<<<<<<" label "Qty TL(Scrap)".
def var cost_tl     as dec format "->>>>>>>9.99<<<<<<"     label "     Cost TL".
def var scrp_tl     as dec format "->>>>>>>9.99<<<<<<"     label "    Scrap TL".
def var ptdesc      like pt_desc1 label "Description".
def var i as int.

{us/xx/xxcsrp7x.i}
/*moved to include ***
def shared temp-table tcost
...
form site csset
with frame pageheader width 80 side-labels no-attr-space page-top.

/* SET EXTERNAL LABELS */
setFrameLabels(frame pageheader:handle).

/*c1429* display site csset with frame pageheader. */
...
and pt_part <= part1 no-lock
 ...
 then
   ...
   find first ps_mstr no-lock use-index ps_parcomp
       ...
       ps_mstr.ps_par = comp and
/*c1429*/ trim(ps_mstr.ps_ps_code) <> "A"
       no-error.
...
/*c1429* start deleted code >>>>
   ...
   with frame det2 width 134 no-attr-space.
*c1429* end deleted code >>>> */
/*c1429* start added code */
   form
      lvl ps_comp format "x(26)" ptdesc qty_per l_rate qty_tl qty_scrp_tl cost_tl scrp_tl 
      qty_type um ps_ps_code
      mtl_tl lbr_tl bdn_tl ovh_tl sub_tl total_tl
   with frame det2 width 300 no-attr-space.
/*c1429* end   added code */
...
/*c1429a* start deleted code >>>
   display getTermLabel("PARENT",6) @ lvl parent @ ps_comp.
   display
/*c1429*      "  " + des3 @ ps_comp */
   ...
   if available sct_det then
   display
      sct_cst_tot @ total_tl with frame det2.

   else
   display
      0 @ total_tl with frame det2.
/*c1429*/   down.
*c1429a* end deleted code <<<<< */
...
/*c1429a* start deleted code >>>>
    ...
    display
       ...
       sct_bdn_tl @ bdn_tl
       sct_ovh_tl @ ovh_tl
       sct_sub_tl @ sub_tl with frame det2.
...
*       sct_bdn_ll @ bdn_tl
*       sct_ovh_ll @ ovh_tl
*       sct_sub_ll @ sub_tl with frame det2.
* 
*    else
*    display
*       0          @ mtl_tl
...
*    then do:
*       down 1.
*       display "  " + des4 @ ps_comp with frame det2.
*    end.
*    if des5 <> ""
*    then do:
*       down 1.
*       display "  " + des5 @ ps_comp with frame det2.
*    end.
   ...
   if bom_code <> parent
      ...
      if des3 <> "" then down 1.
      display
         "  " + getTermLabel("BOM",4) + ": " + bom_code @ ps_comp.
   end.
*c1429* end deleted code */
...
/*c1429*/   trim(ps_mstr.ps_ps_code) <> "A"
      ...
      then do:
	    ...
	    where ps_mstr.ps_domain = global_domain and  
	    ps_mstr.ps_par = comp and
/*c1429*/   trim(ps_mstr.ps_ps_code) <> "A" and
            ps_comp = dpart no-lock no-error.
         ...
         for each pswkfl with frame det2:
            ...
            then do:
               ...
               {us/gp/gpsct08.i &part=dingbats &set=csset &site=site}
            end.
...
/*c1429* start added code */
	    if available sct_det then
            assign
               mtl_ext_tl = (sct_mtl_tl * qty_per)
               lbr_ext_tl = (sct_lbr_tl * qty_per)
               bdn_ext_tl = (sct_bdn_tl * qty_per)
               ovh_ext_tl = (sct_ovh_tl * qty_per)
               sub_ext_tl = (sct_sub_tl * qty_per).
            else
            assign
               mtl_ext_tl = 0.0
               lbr_ext_tl = 0.0
...
/*c1429* start added code */
            l_scrp_pct[level] = scrp_pct.
	    l_scrp = 1.0.
	    do i = 1 to level:
              l_scrp = l_scrp * (1 + (l_scrp_pct[i] / 100)).
	    end.
	    l_scrp = 100 * (l_scrp - 1).
/*c1429* end   added code */
	       ...
	       /*c1429* start added code */
               assign
                  mtl_ext_tl = mtl_ext_tl * (100 / (100 - l_scrp))
                  lbr_ext_tl = lbr_ext_tl * (100 / (100 - l_scrp))
                  bdn_ext_tl = bdn_ext_tl * (100 / (100 - l_scrp))
                  ovh_ext_tl = ovh_ext_tl * (100 / (100 - l_scrp))
                  sub_ext_tl = sub_ext_tl * (100 / (100 - l_scrp)).
	       /*c1429* end   added code */
            end.

            assign
               total_ext = mtl_ext + lbr_ext + bdn_ext + sub_ext + ovh_ext
/*c1429*/      total_ext_tl = mtl_ext_tl + lbr_ext_tl + bdn_ext_tl + sub_ext_tl + ovh_ext_tl
               des4 = ""
               des5 = "".
...
/*c1429*            else down 2. */
            ...
            if transtype = "BM"
            then do:
/*c1429*/      l_qty_per[level] = qty_per.
/*c1429*/      l_rate = 1.0.
/*c1429*/      do i = 1 to level - 1:
/*c1429*/         l_rate = l_rate * l_qty_per[i].
/*c1429*/      end.
/*c1429a* start added code */
               create tcost.
	       assign
	       vrecseq = vrecseq + 1
	       tcseq = vrecseq
...
/*c1429* debug*/  l_rate 
                  ...
                  " " @ sub_tl
                  " " @ total_tl.
*c1429a* end deletd code <<<<< */
            end.

            else do:
/*c1429*/      l_qty_per[level] = qty_per_b.
/*c1429*/      l_rate = 1.0.
/*c1429*/      do i = 1 to level - 1:
/*c1429*/         l_rate = l_rate * l_qty_per[i].
/*c1429*/      end.
...
/*c1429* start deleted code >>>>
            ...
            if available sct_det then
            display
               sct_cst_tot @ total_tl.

            else display 0 @ total_tl.
            ...
            if l_linked then do with frame det2:
               display des8 @ ps_comp with frame det2.
            end.
            ...
            then do:
               ...
               display "  " + joint_label @ ps_comp with frame det2.
            end.
...
/*c1429* start added code * debug */
            /* Display extended totals */

            if not available tcost then do:
               create tcost.
	       assign
	       vrecseq = vrecseq + 1
	       tcleaf = yes
	    ...
	    assign
	    tcqty = (qty_per * l_rate)  * 100 / (100 - l_scrp)
	    tccost = (mtl_ext_tl / qty_per)  * (100 - l_scrp) / 100
	    tccostext = l_rate * mtl_ext_tl
	    tclbr = l_rate * lbr_ext_tl
	    tcovh = l_rate * ovh_ext_tl.
...
/*c1429a* start deleted code >>>>>
            display
	       desc1 @ ptdesc
	       (qty_per * l_rate) format "->>>>>>9.9<<<<<<<<" @ qty_tl
	       ((qty_per * l_rate)  * 100 / (100 - l_scrp))
	          format "->>>>>>9.9<<<<<<<<" @ qty_scrp_tl
	       ((mtl_ext_tl / qty_per)  * (100 - l_scrp) / 100)
	          format "->>>>>>9.9<<<<<<<<" @ cost_tl
	       l_scrp @ scrp_tl
               l_rate * mtl_ext_tl @ mtl_tl
               l_rate * lbr_ext_tl @ lbr_tl
               l_rate * bdn_ext_tl @ bdn_tl
               l_rate * ovh_ext_tl @ ovh_tl
               l_rate * sub_ext_tl @ sub_tl
               l_rate * total_ext_tl @ total_tl.
	       down.
*c1429a* end deleted code <<<<<<<<<< */
