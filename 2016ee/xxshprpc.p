/* xxshprpc.p - SHIPMENT PERFORMANCE REPORT BY REASON CODE              */
/* soshprpc.p - SHIPMENT PERFORMANCE REPORT BY REASON CODE              */
/* Copyright 1986 QAD Inc. All rights reserved.                               */
/* $Id: xxshprpc.p,v 1.2 2018/03/13 17:09:59 gbg Exp $: */
/* $Revision: 1.2 $             */
/*                                                                      */
/* Subprogram to the Shipment Performance Report. This subprogram       */
/* displays shipment performance data by reason code.                   */
/*                                                                      */
...
/* LOCAL VARIABLES */
define variable reason_type_desc as character format "x(35)" no-undo.
...
define temp-table tt_reason_totals  no-undo
      field tt_reason_status       like sttq_desc
      field tt_reason_curr_totals  like shp_ext_price
      field tt_reason_item_totals  like shp_ship_qty
      field tt_reason_ship_totals  as integer
      field tt_reason_shipto       like shp_shipto
      field tt_reason_reason       like shpd_rsn_code
      index tt_reason_ix is unique primary
            tt_reason_status.
...
form
   space(1)
   tt_ship_reason       colon 17
   reason_type_desc     colon 39 no-label
   space(1)
...
form
   tt_reason_status       no-label at 10
   tt_reason_curr_totals  column-label "      Currency"
   status_curr_pct        column-label "Percent"
   space(3)
   tt_reason_item_totals  column-label "    Quantity"
   status_item_um         column-label "UM"
   status_item_pct        column-label "Percent"
   space(3)
   tt_reason_ship_totals  column-label "Shipments"
   status_ship_pct        column-label "Percent"
with frame status_reason width 132 no-box down.

/* SET EXTERNAL LABELS */
setFrameLabels(frame status_reason:handle).


/* FIND GL CONTROL FILE FOR CURRENCY CALCULATIONS */
for first gl_ctrl  where gl_ctrl.gl_domain = global_domain no-lock:
end.

for each shp_hist  no-lock
   where shp_hist.shp_domain = global_domain and
  ((shp_customer_source = cust_source_code  or
   ...
   shp_include ),
   each shpd_hist no-lock where shpd_hist.shpd_domain = global_domain and (
        shpd_tran_id = shp_tran_id and
       (shpd_measurement_type = 1 and shpd_meas_subtype = 1 or
        shpd_measurement_type = 2 and shpd_meas_subtype = 4) ) :
   ...
   assign
      tt_ship_reason = shpd_rsn_code
      tt_ship_reason_type = shpd_measurement_type
...
for each tt_ship_info
   break by tt_ship_reason
         by tt_ship_reason_type
         by tt_ship_site
         by tt_ship_cust
         by tt_ship_shipto
         by tt_ship_item
   ...
   for first shp_hist where shp_hist.shp_domain = global_domain and
      shp_tran_id = tt_ship_tran_id no-lock:
   end.
   ...
   /* GET UM CONVERSION FOR SITE, CUSTOMER AND REASON */
   /* TOTALS BASED ON THE REPORT UM OF MEASURE.       */
   ...
   /* DISPLAY DETAIL */
   if first-of(tt_ship_reason) or
      first-of(tt_ship_reason_type) then do:

      if tt_ship_reason_type = 1 then
         reason_type_desc = (getTermLabel("TIME",35)).
      else
         reason_type_desc = (getTermLabel("QUANTITY",35)).
      ...
      for each tt_reason_totals exclusive-lock:
         delete tt_reason_totals.
      end.
   end. /* If first-of(tt_ship_reason) or */

   for first tt_reason_totals where
      tt_reason_status = tt_ship_time_desc no-lock:
   end.
   if available tt_reason_totals then do:
      assign
         tt_reason_curr_totals = tt_reason_curr_totals + conv_ship_amt
         tt_reason_item_totals = tt_reason_item_totals + report_qty_shipped
         tt_reason_ship_totals = tt_reason_ship_totals + 1.
   end.

   else do:
      create tt_reason_totals.

      assign
         tt_reason_status = tt_ship_time_desc
         tt_reason_curr_totals = conv_ship_amt
         tt_reason_item_totals = report_qty_shipped
         tt_reason_ship_totals = 1
         tt_reason_shipto = shp_shipto
         tt_reason_reason = tt_ship_reason.

      if recid(tt_reason_totals) = -1 then.
   end.
   ...
   if first-of(tt_ship_site) then do:
      ...
      for each tt_site_totals exclusive-lock:
         delete tt_site_totals.
      end.

   end. /* If first-of(tt_ship_site) */
   ...
   if available tt_site_totals then do:
      assign
         tt_site_curr_totals = tt_site_curr_totals + conv_ship_amt
         tt_site_item_totals = tt_site_item_totals + report_qty_shipped
      ...
      if recid(tt_site_totals) = -1 then.
   end.
   ...
   if first-of(tt_ship_cust) then do:
      ...
      for each tt_cust_totals exclusive-lock:
         delete tt_cust_totals.
      end.

   end. /* If first-of(tt_ship_cust) */
   ...
   if available tt_cust_totals then do:
      assign
         tt_cust_curr_totals = tt_cust_curr_totals + conv_ship_amt
         tt_cust_item_totals = tt_cust_item_totals + report_qty_shipped
      ...
      if recid(tt_cust_totals) = -1 then.
   end.
   ...
   if first-of(tt_ship_item) then do:
      ...
      for each tt_item_totals exclusive-lock:
         delete tt_item_totals.
      end.
   end. /* If first-of(tt_ship_item) */
   ...
   for first tt_item_totals where
      tt_item_status = tt_ship_time_desc no-lock:
   end.

   if available tt_item_totals then do:
      assign
         tt_item_curr_amts   = tt_item_curr_amts + conv_ship_amt
         tt_item_count       = tt_item_count + conv_qty_shipped
      ...
      if recid(tt_item_totals) = -1 then.
   end.

   /* COLLECT CURRENCY/UM TOTALS */
   if first-of(tt_ship_currency)
   then do:
      for each tt_um_curr_totals exclusive-lock:
         delete tt_um_curr_totals.
      end.
   end.
   ...
   if report_detail then do:
      for first shpd_hist no-lock where shpd_hist.shpd_domain = global_domain
         and shpd_tran_id = shp_tran_id and
...
/*cy1044* start added code */
      vShp_open_qty = if cust_source = "SO" then
                         shp_open_qty + shp_ship_qty
                      else shp_open_qty.
      vShp_ext_price = shp_ext_price.
                      /* if cust_source = "SO" then
                         shp_ship_price + shp_ext_price
                      else shp_ext_price. */
      ...
      qty_pct = shp_qty_pct.
                /* if cust_source = "SO" then
                      ((shp_ship_qty / (shp_ship_qty + shp_open_qty)) * 100) 
		            - 100
		else shp_qty_pct. */
...
/*cy1044*/ (if include_amt then vshp_ext_price else 0) @ shp_ext_price
   ...
   end. /* IF LAST-OF (tt_ship_currency */
   ...
   if last-of(tt_ship_item) then do:
    ...
    /* INITIALIZE VARIABLES */
      ...
      for each tt_item_totals no-lock
         ...
         display
            tt_item_status
            (if include_amt then tt_item_curr_amts else 0) @
                tt_item_curr_amts
            (if include_amt then status_curr_pct else 0) @
                status_curr_pct
            tt_item_count
            status_item_um
            status_item_pct
   ...
   if last-of(tt_ship_cust) then do:
    ...
    /* INITIALIZE VARIABLES */
      ...
      for each tt_cust_totals no-lock
         ...
         display
            tt_cust_status
            (if include_amt then tt_cust_curr_totals else 0) @ tt_cust_curr_totals
      ...
      display
         ...
         (if include_amt then total_curr_amts else 0) @ tt_cust_curr_totals
         (if include_amt then total_pct else 0) @ status_curr_pct
    ...
    /* INITIALIZE VARIABLES */
      ...
      display
         ...
         (if include_amt then total_curr_amts else 0) @
             tt_site_curr_totals
         (if include_amt then total_pct else 0) @
             status_curr_pct
   ...
   /* DISPLAY TOTALS FOR REASON */
   if last-of(tt_ship_reason) or
      last-of(tt_ship_reason_type) then do:
    ...
    /* INITIALIZE VARIABLES */
      ...
      for first tt_reason_totals no-lock:
      end.

      if available tt_reason_totals then do:
         totals_label3 = (getTermlabel("REASON_TOTALS",35)) + ":  "
                         + tt_reason_reason.
      ...
      for each tt_reason_totals no-lock:
         total_item_count  = total_item_count + tt_reason_item_totals.
         total_curr_amts   = total_curr_amts + tt_reason_curr_totals.
         total_ship_count = total_ship_count + tt_reason_ship_totals.
      end. /* FOR EACH tt_reason_totals */

      for each tt_reason_totals no-lock
         break by tt_reason_status:

         assign
            status_curr_pct  = if tt_reason_curr_totals > 0 then
                              (tt_reason_curr_totals /
                               total_curr_amts) * 100
                               else 0
            status_item_pct  = if tt_reason_item_totals > 0 then
                               (tt_reason_item_totals /
                               total_item_count) * 100
                               else 0
            status_ship_pct = if tt_reason_ship_totals > 0 then
                              (tt_reason_ship_totals /
                              total_ship_count) * 100
                              else 0.

         display
            tt_reason_status
            (if include_amt then tt_reason_curr_totals else 0)
                @ tt_reason_curr_totals
            (if include_amt then status_curr_pct else 0) @
                status_curr_pct
            tt_reason_item_totals
            ...
            tt_reason_ship_totals
            status_ship_pct
         with frame status_reason.
         down with frame status_reason.

      end. /* FOR EACH tt_reason_totals */

      display
         (getTermlabel("TOTALS",24)) @ tt_reason_status
         (if include_amt then total_curr_amts else 0) @
             tt_reason_curr_totals
         (if include_amt then total_pct else 0) @
             status_curr_pct
         total_item_count @ tt_reason_item_totals
         status_item_um @ status_item_um
         total_pct @ status_item_pct
         total_ship_count @ tt_reason_ship_totals
         total_pct @ status_ship_pct
      with frame status_reason.
      down 2 with frame status_reason.

   end. /* IF LAST-OF(tt_ship_reason) */
